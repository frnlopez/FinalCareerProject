...
- **Citas nuevas: `[CITA: autor o tema]`, NUNCA un `[n]` que te inventes.** Los `[n]` son un contador **global** que apunta a `Bibliografía.md`, y hoy ya están **en uso `[1]`–`[8]` y `[10]`** (verificado contra `Bibliografía.md` el 2026-08-14); **`[9]` está retirada SIN renumerar, o sea quemada y no libre**, y el primer número disponible sería el `[11]`. Ojo con `[2]` (Anderson 1980) y `[3]` (Denning 1987), que son las que más vas a necesitar en el capítulo 2: **ya existen, se mapean**. Si asignas un número nuevo por tu cuenta y hay otro redactor trabajando en paralelo, los dos empezáis en el mismo número y el solape no lo detecta nadie al leer. El paso de `[CITA: …]` a `[n]` es un pase **posterior y en serie** del `researcher`. Un `[n]` que **ya estaba** en la nota se respeta tal cual: no lo renumeras ni lo borras. **`Bibliografía.md` no lo escribes tú.**
