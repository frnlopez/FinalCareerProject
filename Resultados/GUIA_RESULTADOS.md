...
| `validacion.py` | ejecutado (54 y 122) | **Por variante**: un `..._validation_report.txt`, un `..._vocabulario_onehot.csv` (§2.4), un `..._composicion_d3.csv` (§2.5) y **6** figuras de validación (las 4 de siempre + las 2 del KS contra los normales de D2, tarea T2). Con las dos variantes: **2** informes (`specialized_nsl_kdd_validation_report.txt` y `specialized_nsl_kdd_sin_seleccion_validation_report.txt`), **2** CSV de vocabulario, **2** CSV de composición de D3 y **12** figuras `validacion_*` |
...
| `agregar_semillas.py` | **corrido cinco veces** (T4: **7,2 s** la primera y **1,70 s** la segunda, ambas el 2026-08-13, esta tras el arreglo de `_tabla_md()`; una tercera con el **titular automático** (2026-08-13 `19:54:48`); una cuarta con el **reparto por commit de origen del titular** (2026-08-14 `15:15:13`); y una quinta con el **recuento pareado por semilla** (2026-08-17). **En disco están los de la quinta**) | `dispersion_semillas.csv`, `dispersion_semillas.md` y **`comparaciones_pareadas.csv`** — los dos primeros con **198 filas** (98 de *calidad* + 100 de *dispersión de máquina*) con n, media, sd muestral (`ddof=1`), mín y máx, más el valor de la semilla 42 al lado de cada banda y el recuento **«13 de 98»** fuera de banda; el tercero con **13 filas de datos** de recuentos pareados. `commits_origen = df30cb2` único en todas las celdas y `commit_agregador = 0276039-sucio` — sello impreso, que **queda intacto** y está **re-anclado en prosa a `98a0289`** (§6.2); las pasadas segunda, tercera y cuarta quedaron re-ancladas a `9ad971b`, `9d4c26d` y `1cb5c26` respectivamente. **Ninguna de las cinco pasadas de este agregador tiene re-anclaje pendiente** (verificado el 2026-08-17). El re-anclaje que arrastraba `validacion.py` (`a8c20e9-sucio`, 2026-08-16) **también está cerrado**, y no por prosa sino re-corriendo el script con el árbol limpio el 2026-08-17: su sello es hoy `ac36b88`, limpio (§2.4) |
...
| `commit`, `fecha` | procedencia de la corrida, **al final de la fila**; mismo mecanismo y misma convención `-sucio` que la columna `commit` de los `metricas_*.csv` (`config.commit_actual()`). **Presentes en el CSV desde la corrida del 2026-08-11 20:53; el que hay hoy en disco es el de la re-corrida con el árbol limpio del 2026-08-17** (sello `ac36b88`, **sin** `-sucio`): ver el aviso de abajo sobre qué vale y qué no vale un sello sucio, y por qué este no lo es |
...
- **Los dos ficheros traen las mismas cifras**, y es correcto: la medición se hace sobre los CSV
  `_original_*`, y la selección de características actúa **después** del one-hot. Es **una de las dos**
  salidas de `validacion.py` que no dependen de la variante: la otra es el `..._composicion_d3.csv`
  de **§2.5**, que cuenta filas de D3 y por eso tampoco cambia con el set de características.
  *(Hasta la re-corrida del 2026-08-16 era la única; el alta de §2.5 lo dejó desfasado.)*
- **El CSV que hay en disco YA lleva `commit` y `fecha`**, igual que los `metricas_*.csv`. El
  ...
  `config.commit_actual()`) y las dos variantes **se han vuelto a correr**: los ficheros publicados
  salen de la re-corrida con el árbol limpio del **2026-08-17**, con sello **`ac36b88`** (sin
  `-sucio`) en la columna `commit` y, en
  cada fila, la fecha de **su propia** invocación en la columna `fecha`. **Las cifras del
  vocabulario no cambian** respecto a la
  corrida del 2026-08-11 (`fc1c6b4-sucio`): son las mismas mediciones y **solo cambia el
  sello**. La cabecera de los dos `..._validation_report.txt` trae **el
  mismo commit**, y su propia marca temporal en `Fecha de la corrida:`: el commit
  sí es común a los seis artefactos de la corrida, la fecha no, porque se captura una vez por invocación —cada
  variante lleva la suya. **La marca temporal no se copia a la prosa**: se remite al campo del
  artefacto, porque cualquier re-corrida futura la cambia y dejaría desfasado lo escrito aquí; el
  `commit`, en cambio, sí se cita, porque identifica el estado del código y no cambia al re-correr.
  **Esas columnas se pueden y se deben citar**: son
  ...
  reconstruir el código que lo produjo. **Esa salvedad no aplica a los ficheros que hay hoy en
  disco**: la primera pasada de este ciclo (2026-08-16) sí salió `a8c20e9-sucio`, pero se rehízo el
  **2026-08-17 con el árbol limpio** y quedó sobrescrita, con las mismas cifras. `ac36b88` es el
  commit **anterior** al que versiona estos artefactos, pero al no haber cambios sin commitear
  **identifica exactamente el código** que los produjo y la corrida es **reproducible** desde él.
  El recuadro de anclaje de `Implementacion/PIPELINE.md` sigue siendo el registro canónico de la
  corrida.
...
### 2.5 Los dos `..._composicion_d3.csv` (los genera `validacion.py`)

Artefacto **nuevo de la re-corrida del 2026-08-16**: pone en disco el **reparto de D3 por categoría
de ataque**, que hasta ahora solo se citaba en prosa. Una fila por categoría (`dos`, `probe`, `r2l`,
`u2r`) más una fila `__total__`. Su gemelo en el informe es el bloque «Composición de D3 por
categoría de ataque», que apunta al propio CSV como fuente.

| Columna | Qué es |
|---|---|
| `variante` | `con_seleccion` / `sin_seleccion` |
| `split` | siempre `D3` — la columna está para que el fichero se lea suelto sin suponer el split |
| `categoria` | la categoría de ataque, o `__total__` en la fila de agregados |
| `n_instancias` | recuento de filas de D3 en esa categoría; **entero** |
| `porcentaje_d3` | porcentaje sobre el total de D3, con **4 decimales** |
| `declarada_en_config` | `True` si la categoría está entre las que declara `config.py`; **vacío en la fila `__total__`**, que no es una categoría |
| `ratio_desbalance_max_min` | mayoritaria ÷ minoritaria; **solo en la fila `__total__`**, vacío en las demás |
| `commit`, `fecha` | procedencia de la corrida, **al final de la fila**; mismo mecanismo y misma convención `-sucio` que en §2.4 |

Cifras publicadas (**idénticas en los dos ficheros**, ver aviso abajo):

| Categoría | `n_instancias` | `porcentaje_d3` |
|---|---|---|
| `dos` | 45.927 | 78,3336 % |
...
`ratio_desbalance_max_min` = **883,21:1** (`dos` frente a `u2r`).

Dos avisos para citarlo:

- **Los dos ficheros traen las mismas cifras, y es correcto.** El reparto cuenta **filas** de D3, y
  la selección de características actúa sobre las **columnas**: no puede alterar cuántas instancias
  hay de cada categoría. Es, junto al del vocabulario (§2.4), la otra salida de `validacion.py` que
  no depende de la variante — y el propio informe lo rotula así.
- **Los porcentajes suman 100 solo hasta el redondeo a 4 decimales**; los recuentos sí suman el
  total exacto (45.927 + 11.656 + 995 + 52 = 58.630). Si en la memoria se presenta la tabla, el
  total que se cita es el de instancias, no la suma de los porcentajes.

Procedencia de los ficheros en disco: re-corrida con el árbol limpio del **2026-08-17**, sello
**`ac36b88`** en la columna `commit` de los dos CSV; la marca temporal exacta —distinta en cada
variante— la declara cada fichero en su columna `fecha`, y **no se reproduce aquí** para que una
re-corrida no desfase esta guía. Al ser el sello limpio, **no hay re-anclaje pendiente**: identifica
por sí solo el código que generó los CSV — misma situación que en §2.4.

**Destino en la memoria:** **4.2.1** (análisis previo de la BD, desbalance de clases) y **4.3.4**
(mini-experimento de balanceo), como respaldo en disco del ratio que motiva SMOTE / `class_weight`.
...
que hay en disco**; el detalle, en el aviso al final de este apartado y en §2.4)* · el titular de integridad y tamaños · **la composición de D3 por
categoría de ataque** · las dos mediciones de drift y su
...
este apartado; el del vocabulario tiene además CSV propio (**§2.4**). El de la composición de D3 es
**nuevo de la re-corrida del 2026-08-16** y tiene también CSV propio (**§2.5**): va justo después
del titular de tamaños y **no altera ninguna de las cifras anteriores**.
...
> la re-corrida con el árbol limpio del **2026-08-17** (exit 0, integridad **APROBADA** en las dos
> variantes): en `specialized_nsl_kdd_validation_report.txt`, los campos `Commit del código:` y
> `Fecha de la corrida:` declaran, respectivamente, el commit `ac36b88` —común a las dos
> variantes— y la marca temporal de esa invocación, distinta en cada una. **La marca temporal no se
> copia aquí**: se consulta en el campo, porque caduca en cuanto se vuelve a correr; el commit sí se
> cita, porque no cambia al re-correr. Los dos campos se citan **por su nombre
...
> cerrado.** Y **sin el límite propio del sufijo**: un sello `-sucio` no identificaría la versión del
> código (el hash sería el del commit *anterior* al cambio; `-sucio` solo dice «difería, no se sabe
> en qué»), pero **este sello no lo lleva**. La primera pasada de este ciclo, del 2026-08-16, sí
> salió `a8c20e9-sucio` porque el cambio de `validacion.py` que produce el bloque de composición de
> D3 estaba sin commitear; se rehízo con el árbol limpio y esa pasada quedó sobrescrita, con las
> mismas cifras. `ac36b88` sigue siendo el commit **anterior** al que versiona los artefactos, pero
> **identifica exactamente el código** que los generó. El recuadro de trazabilidad de
> `Implementacion/PIPELINE.md` es el registro canónico de la corrida.
...
(2026-08-13). El agregador se ha ejecutado **cinco veces**, todas con cero `fit` —solo lee CSV—: la
...
`.md` (`6bb224c-sucio`, `2026-08-14T15:15:13`); y la quinta con el **recuento pareado por semilla**,
que estrena `comparaciones_pareadas.csv` y añade al `.md` la sección «Comparaciones pareadas por
semilla». **Los ficheros del agregador que hay en disco son los de la quinta, y ya son TRES**:
`commit_agregador = 0276039-sucio`, verificado en disco el 2026-08-17 (la marca temporal la declara la
cabecera del propio artefacto).
...
- **Cuarta pasada, re-anclada:** `6bb224c-sucio` → **`1cb5c26`** («codigo+informe: cerrar la Fase 2 -
  Tanda 2, ficha del sistema y reconciliacion de sellos», 2026-08-14), el commit que versiona a la vez
  `agregar_semillas.py` y los dos ficheros de dispersión, que venían sin commitear de la sesión
  anterior (verificado con `git log --oneline 1cb5c26` el 2026-08-14, no deducido de la prosa).
  `6bb224c` es igualmente el
  commit **anterior** al cambio, así que `6bb224c-sucio` no identifica la versión que produjo la
  agregación: la identifica `1cb5c26`. Fue el **quinto** re-anclaje del proyecto. **Ya no es la pasada
  que hay en disco** —la sobrescribió la quinta, del 2026-08-17—, pero **sigue nombrando su pasada** y
  no se borra. **Con esto, y a fecha 2026-08-14, no quedaba ningún re-anclaje pendiente.**
  *(Estuvo **en suspenso entre el 2026-08-16 y el 2026-08-17**: la re-corrida de `validacion.py` del
  2026-08-16 publicó seis artefactos con sello `a8c20e9-sucio`. Ese pendiente se cerró **sin
  re-anclaje**, re-corriendo el script con el árbol limpio el 2026-08-17: su sello es hoy `ac36b88`,
  y un sello limpio no necesita anclarse (§2.4). Cada re-anclaje enumerado en
  las viñetas de esta lista **sigue siendo cierto**, uno por uno. Aquí no se cuenta cuántos son —el total lo da la **última**
  viñeta de la lista, que es la que se actualiza al añadir uno nuevo.)*
- **Quinta pasada, re-anclada:** `0276039-sucio` → **`98a0289`** («codigo: emitir el "8 de 10" pareado
  desde agregar_semillas.py y corregir el residuo de T22», 2026-08-17), el commit que versiona a la vez
  el cambio de `agregar_semillas.py` y los **tres** artefactos del agregador, que salieron de código
  sin commitear (verificado con `git show --stat 98a0289` el 2026-08-17, no deducido de la prosa).
  `0276039` es el commit **anterior** al cambio —y es de track *informe*, no de código—, así que
  **`0276039-sucio` no contiene el código que produjo los artefactos**: lo contiene `98a0289`. Es el
  **sexto** re-anclaje del proyecto, y es la pasada que hay en disco. El pendiente de `validacion.py`
  (`a8c20e9-sucio`) **no lo cerró este re-anclaje, sino la re-corrida del 2026-08-17**: sus seis
  artefactos se regeneraron sobre árbol limpio y su sello es hoy `ac36b88`, sin sufijo, de modo que
  **no queda re-anclaje pendiente** (§2.4).
...
lleva** desde la cuarta pasada y lo sigue llevando en la quinta, verificado en el fichero el
2026-08-17. Lo que hay en disco:
...
- **Última actualización: 2026-08-17** (**cero corridas, cero `fit`, cero artefactos regenerados y
  cero cifras alteradas**: solo texto). **Sexto re-anclaje de sello del proyecto:** `0276039-sucio` →
  **`98a0289`**, el de la **quinta** pasada del agregador (la que estrenó `comparaciones_pareadas.csv`).
  - `0276039` es un commit de track *informe* («informe: separar en 3.5 los dos "8 de 10" homonimos») y
    es el **anterior** al cambio, así que **no contiene el código que produjo los artefactos**. El que
    sí lo contiene es `98a0289` («codigo: emitir el "8 de 10" pareado desde agregar_semillas.py y
    corregir el residuo de T22»), que versiona a la vez `agregar_semillas.py` y los tres artefactos.
    Verificado con `git show --stat 98a0289`, no deducido de la prosa.
  - **Reconcilia además la prosa con el fichero en disco:** §1 y §6.2 decían que en disco estaban los
    **dos** artefactos de la **cuarta** pasada (`6bb224c-sucio`, `2026-08-14T15:15:13`), y en disco
    están los **tres** de la quinta. Las 198 filas y el «13 de 98» **no cambian**.
  - Se escribe en **cinco emplazamientos** —los cuatro de siempre **más `A.3`**—: §1 y §6.2 de esta
    guía, y la tabla de corridas más
    el recuadro de re-anclajes de `PIPELINE.md` (que gana además la fila de la 5.ª pasada y la ficha
    del runbook de `comparaciones_pareadas.csv`), y **`A.3 Ficha del sistema`**, que también lo tiene
    escrito y se olvida si uno cuenta cuatro. El sello impreso
    **dentro** de los artefactos **no se toca**: lo estampa `config.commit_actual()`.
  - **No es este re-anclaje el que cierra el pendiente de `validacion.py`** (`a8c20e9-sucio`): lo
    cerró la re-corrida del 2026-08-17 sobre árbol limpio, que dejó los seis artefactos con sello
    `ac36b88` **sin sufijo `-sucio`**. Un sello limpio no necesita anclarse, así que **ya no hay
    ningún `> [!todo]` de este sello en §2.4** — ver la nota de §2.4 sobre por qué no hay re-anclaje
    pendiente.
- **Actualización anterior: 2026-08-16** (**una re-corrida de `validacion.py` en las dos variantes,
  cero `fit`, cero cifras de calidad alteradas**). `validacion.py` pasa a emitir el **reparto de D3
  por categoría de ataque**, y con ello:
  - **Alta de artefacto:** los dos `..._composicion_d3.csv`, inventariados en **§2.5** y añadidos a
    la fila de `validacion.py` de §1. Bloque nuevo homónimo en los dos `..._validation_report.txt`
    (§3.2).
  - **Re-corrida de las dos variantes** el 2026-08-16, exit 0 e integridad **APROBADA**. Sello
    interno `a8c20e9-sucio` en los seis artefactos; la fecha, distinta por variante, la declara cada
    uno en su campo `Fecha de la corrida:` (informes) o su columna `fecha` (CSV). Los
    dos `..._vocabulario_onehot.csv` se reescriben con **las mismas mediciones** (77 → 122, delta
    45): **solo cambia el sello**. Las 12 figuras `validacion_*.png` se regeneran igual. Drift,
    outliers y tamaños **no cambian**: (A) 37 / (B) 25 en 54 y (A) 44 / (B) 31 en 122.
  - **Sello pendiente:** el commit de cierre de esta re-corrida **todavía no existe**; el `-sucio`
    es previsto (el cambio de `validacion.py` estaba sin commitear al correr). Queda marcado con
    `> [!todo]` en §2.4 de esta guía y en el recuadro de trazabilidad de
    `Implementacion/PIPELINE.md`, que es donde se sellará. **No se escribe hash mientras tanto.**
    Con esto vuelve a haber **un re-anclaje pendiente** en el proyecto, tras el periodo en que no
    quedaba ninguno.
    *(Glosa de cierre: «el commit de cierre todavía no existe», «queda marcado con `> [!todo]` en
    §2.4» y «vuelve a haber un re-anclaje pendiente» son afirmaciones **del 2026-08-16**, ciertas
    aquel día y **superadas el 2026-08-17**: la re-corrida sobre árbol limpio dejó los seis
    artefactos con sello **`ac36b88`** sin sufijo, se retiró el `> [!todo]` de §2.4 y no quedó
    re-anclaje pendiente. Esta viñeta es historial y no se reescribe.)*
- **Añadido el 2026-08-14, tercera pasada del día** (**cero corridas, cero `fit`, cero
  ...
  - **Renumeración:** los re-anclajes hechos pasan de tres a **cuatro** y el pendiente pasa de dos a
    **uno**: el de la **cuarta** pasada (`6bb224c-sucio`, 2026-08-14 `15:15:13`), cuyo commit de
    cierre efectivamente **no existe todavía**. Ahí no se escribe ningún hash. *(**Corregido el
    2026-08-14, cuarta pasada**: ese commit de cierre ya existe, es **`1cb5c26`**, y el re-anclaje
    `6bb224c-sucio` → `1cb5c26` —el **quinto** del proyecto— ya está hecho en §6.2 y en la tabla de
    scripts. **No queda ningún re-anclaje pendiente.** El sello impreso sigue intacto.)*
- **Añadido el 2026-08-14, segunda pasada del día** (**cero corridas de modelos, cero
  ...
  - **Ni el sello de la tercera pasada ni el de la cuarta se re-anclan todavía**: sus commits de
    ...
    **tercera** pasada era falso — su commit de cierre sí existía, es `9d4c26d`, y el re-anclaje ya
    está hecho. Ver la entrada de arriba.)* *(**Corregido el 2026-08-14, cuarta pasada**: lo dicho
    aquí de la **cuarta** pasada también ha quedado superado — su commit de cierre es **`1cb5c26`** y
    el re-anclaje `6bb224c-sucio` → `1cb5c26` ya está hecho, así que **ninguno de los dos sellos
    sigue pendiente**.)*
- **Añadido el 2026-08-14, primera pasada del día** (**cero corridas, cero `fit`, cero artefactos
  ...
  - **Sello pendiente:** el commit de cierre de esta pasada aún no existe; queda **pendiente** de
    ...
    sigue pendiente el de la cuarta, y el de esta entrada.)* *(**Al día el 2026-08-14, cuarta
    pasada**: el de la **cuarta** pasada del agregador tampoco está ya pendiente —es **`1cb5c26`**, y
    el re-anclaje ya está hecho—. Y **esta entrada tampoco tiene re-anclaje pendiente**: esta
    pasada fue **solo texto** y **no estampó ningún sello en ningún artefacto**, así que no hay
    nada que re-anclar. Por eso no se escribe hash aquí: no es un pendiente, es un **no-sello**.
    Con esto, **no queda ningún re-anclaje pendiente** en el proyecto.)*
...
- Actualización anterior: **2026-08-11** (residuos del cierre de `validacion.py`, solo rótulos,
  ...
  trazabilidad» de `Implementacion\PIPELINE.md`, reescrito en la misma pasada: **en aquella fecha**
  los informes en disco eran los de la re-corrida del **2026-08-11 a las 20:53** (sello interno
  `fc1c6b4-sucio`), no los de T2 del 2026-08-10. **Eso quedó superado por la re-corrida del
  2026-08-16**, que es la que hay hoy en disco (ver la entrada de esta misma sección fechada el
  2026-08-16, arriba). **Cerrado también** el residuo de `commit`+`fecha`: se estampan ya en la
  ...
  **`9af842c`** y así lo declaran hoy el recuadro de trazabilidad y la tabla de corridas de
  ...
  artefactos **no se editó**: es una salida generada, y **hasta el 2026-08-16 decía**
  `fc1c6b4-sucio`. La re-corrida del 2026-08-16 lo reescribió a `a8c20e9-sucio`, así que **ese
  re-anclaje sigue siendo válido para la corrida que nombra —la del 2026-08-11— pero ya no
  describe lo que hay en disco**; lo que hay en disco lo documenta la entrada del 2026-08-16 al
