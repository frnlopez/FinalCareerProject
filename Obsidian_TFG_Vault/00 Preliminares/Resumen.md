...
> [!question] Candidatos de título — decide Francisco
> 1. **Sistema híbrido de detección de intrusiones en red basado en aprendizaje automático: detección de anomalías y firmas en cascada sobre NSL-KDD** — el más técnico y completo; nombra arquitectura, técnica y dataset, a costa de la longitud.
> 2. **Detección de ataques 0-day en redes mediante un H-NIDS en cascada sobre NSL-KDD** — pone el resultado diferencial (0-day) en el titular; mantiene «híbrido» de forma abreviada y conserva el dataset.
> 3. **Diseño y evaluación de un sistema híbrido de detección de intrusiones con aprendizaje automático** — registro clásico de TFG («diseño y evaluación»), sin mención al dataset; el más neutro y el más fácil de defender si el alcance se reformula.
> 4. **Más allá de las firmas: aprendizaje automático para detectar ataques desconocidos en redes** — el más divulgativo, sin «híbrido» ni NSL-KDD; comunica la motivación pero no delata la arquitectura.
> 5. **Detección de intrusiones en dos etapas: anomalías y firmas combinadas mediante aprendizaje automático** — evita la palabra «híbrido» y la sustituye por la descripción literal de la arquitectura, que es más informativa para un lector no especialista.
