...
- El `±` es `f1_macro_cv_std`: la **desviación típica entre esos 5 *folds* de esa misma corrida**. Es dispersión **intra-corrida**, y por tanto **no** es la dispersión entre semillas del barrido de `5.2.4` (que vive en `metricas_balanceo_semillas.csv` y no se mezcla aquí).
...
Y, medido con la **misma vara** que el eje anterior —el barrido de diez semillas, no la corrida única—, el resultado se sostiene: de las **40 celdas** de este eje (2 variantes × 2 algoritmos × 10 semillas) de `metricas_balanceo_semillas.csv`, **SMOTE gana en 40 de 40 (100 %)**. No hay ninguna semilla, variante ni algoritmo en que no aplicar balanceo supere a aplicarlo. El contraste con el otro eje es directo y se lee sobre denominadores idénticos:
