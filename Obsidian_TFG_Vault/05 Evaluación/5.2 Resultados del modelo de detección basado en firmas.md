...
- **Comparación pareada (semilla a semilla):** enfrentando ambos algoritmos **dentro de cada semilla**, **RandomForest gana en 8 de las 10**. Esta es la formulación que sostiene la elección de RandomForest como etapa 2, y es la única que debe usarse al afirmar que un algoritmo es mejor que el otro.
...
> [!todo] Falta respaldo automatizado del 40/40
> El reparto del eje **SMOTE vs `class_weight`** (17 de 40) sí lo emite el agregador y se puede repuntar a `Resultados\dispersion_semillas.md:266-276`. El **40/40 de este eje no está respaldado por ningún artefacto de `Resultados/`**: el agregador solo declara las columnas de decisión **no constantes**, y este eje, al ser unánime, no genera línea. Mientras no se emita el recuento por script —tiene ficha aparte—, la cifra queda pendiente de verificación y **no debe citarse como recuento automatizado**, a diferencia del 13 de 98 de 5.2.4.3. El contraste con el otro eje es directo y se lee sobre denominadores idénticos:
...
> [!info] Trazabilidad
> Dispersión: `Resultados\dispersion_semillas.md` y `Resultados\dispersion_semillas.csv` (agregados por `agregar_semillas.py`; los dos ficheros en disco llevan `commit_agregador = 6bb224c-sucio`), a partir de `metricas_firmas_semillas.csv`; volcado íntegro en [[A.3 Ficha del sistema|A.3]]. Balanceo: `Resultados\metricas_balanceo.csv` (16 filas, sello `1163c90`; columnas `balanceo`, `f1_macro_cv` y `f1_macro_cv_std`, todas con `semilla=42`). Recuento entre semillas del eje SMOTE vs `class_weight` (17 de 40 celdas para `class_weight`): `Resultados\dispersion_semillas.md`, líneas 266-276, donde el agregador lista celda a celda el reparto de la columna de decisión `balanceo` en los cuatro pares variante×algoritmo. Ejes de balanceo por algoritmo: `firmas.py:103-108` (`BALANCEO_OPCIONES`); particionado dependiente de la semilla: `firmas.py:40`.
