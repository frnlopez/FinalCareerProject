...
**Etapa 2 (firmas).** Sobre los ataques de tipo conocido, la configuración seleccionada es **RandomForest con SMOTE** (f1_macro = 0.822 sobre D2, cifra de la corrida publicada con semilla 42). Conviene enunciar con precisión en qué sentido supera a HistGradientBoosting, porque el barrido de diez semillas de [[A.3 Ficha del sistema|A.3]] muestra que **el orden entre ambos no queda establecido en términos absolutos**: sus bandas se solapan y el máximo de HistGradientBoosting queda por encima de la media de RandomForest. Lo que sí sostiene la elección es la **comparación pareada** —RandomForest gana en **8 de las 10 semillas**—, y así es como debe leerse aquí y en [[5.2 Resultados del modelo de detección basado en firmas|5.2]], que publica las bandas. Las categorías `dos` y `probe` están esencialmente resueltas por todos los algoritmos; `r2l` queda en un nivel intermedio; y **`u2r` es el punto débil universal** (F1 ≤ 0.452), un límite impuesto por su soporte mínimo (~52 muestras de entrenamiento) que ni SMOTE ni el ajuste por coste (`class_weight`) compensan del todo. Esta etapa arrojó además un resultado metodológico relevante: la **selección de características a 54 (§4.3.5) no solo redujo la dimensionalidad, sino que mejoró la generalización** D3→D2 del clasificador, al eliminar los grados de libertad con los que RandomForest sobreajustaba las 122 características (caída de 0.822 a 0.696 con el conjunto completo, invisible en validación cruzada).
