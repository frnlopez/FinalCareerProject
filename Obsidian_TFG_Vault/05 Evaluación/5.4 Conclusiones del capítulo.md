...
4. **El algoritmo ganador de cada etapa se eligió con métricas medidas sobre D2** (H-2 para el Autoencoder, H-3 para el RandomForest). La regla anti-fuga del proyecto es acotada —D2 no se usa para ajustar hiperparámetros ni umbrales, y eso se respetó— pero no cubre esa comparación final: es una selección de modelo sobre el conjunto de test, y las cifras de los dos ganadores deben leerse como optimistas por selección. Se declara aquí y en [[5.1 Resultados del modelo de detección de anomalías|5.1]] y [[5.2 Resultados del modelo de detección basado en firmas|5.2]] antes que omitirse.
