...
El único trabajo hermano verificable en detección de 0-day sobre NSL-KDD propone un autoencoder con One-Class SVM como baseline —la misma pareja de la etapa 1 de este sistema— y reporta una «*zero-day detection accuracy*» del 89-99 % [CITA: Hindy et al. 2020, «Utilising Deep Learning Techniques for Effective Zero-Day Attack Detection», *Electronics* 9(10):1684, tabla 3 y §5]. **Ese 89-99 % no puede ponerse al lado del 0,771 de recall 0-day de este trabajo**, porque incumple C7 por tres motivos que no son tecnicismos:
...
La causa mecánica es directa y está en el código: cambiar la semilla cambia el particionado de `StratifiedKFold`, de modo que tanto el ganador del mini-experimento de balanceo como la configuración elegida por el *grid* pueden cambiar con ella. **El detalle numérico se publica una sola vez, en [[5.2 Resultados del modelo de detección basado en firmas|5.2]]**, y no se repite aquí. Conviene precisar qué contiene esa sede, porque los dos ejes no se documentan con el mismo nivel de detalle:
...
| SMOTE vs *nada* (KNN, HistGradientBoosting) | **2 celdas**, únicamente las de HistGradientBoosting (2 variantes) | No |
...
> [!todo] Las dos celdas de KNN del eje SMOTE vs *nada* no están publicadas en `5.2` como cifra individual; solo entran en el recuento pareado agregado. Si se quisieran tabular, habría que volcarlas desde `metricas_balanceo.csv`. La configuración efectivamente publicada es idéntica en las dos variantes de características: DecisionTree con `class_weight`; RandomForest, KNN e HistGradientBoosting con SMOTE.
