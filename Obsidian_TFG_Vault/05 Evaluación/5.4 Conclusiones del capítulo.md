...
2. **La cascada tiene un coste sobre lo conocido.** El clasificador de firmas aislado lograba f1_macro = 0.8223 sobre los ataques conocidos —titular de la semilla 42, cuya banda entre semillas es **[0,7779–0,8205]** con media **0,8035**—; la cascada end-to-end obtiene 0.748. Esos ~7,4 puntos son el precio estructural de filtrar por la etapa 1: es el intercambio deliberado de sacrificar rendimiento sobre lo conocido a cambio de la capacidad de ver lo desconocido.
...
4. **Tres decisiones del sistema se tomaron mirando D2**: el detector de la etapa 1 (H-2, Autoencoder), el clasificador de la etapa 2 (H-3, RandomForest) y **el set de características, 54 frente a 122** (Q1/C, experimento H1, decidido por recall 0-day por tipo y `f1_macro` medidos sobre D2; el enunciado del experimento y su criterio de decisión tienen su sede canónica en [[4.3 Preprocesamiento de los datasets|4.3.5]] § «Decisión experimental: 54 frente a 122 (experimento H1, medido sobre D2)», y esta nota no los redefine). La regla anti-fuga del proyecto es acotada —D2 no se usa para ajustar hiperparámetros, balanceo ni umbrales, y eso se respetó— pero no cubre esas comparaciones. Las tres son selección de modelo sobre el conjunto de test, es decir *data snooping*, y por tanto las métricas publicadas son optimistas respecto a lo que daría un test verdaderamente ciego. Se declara aquí como cierre del capítulo de evaluación, y **cada una en su etapa**, en [[5.1 Resultados del modelo de detección de anomalías|5.1]] y [[5.2 Resultados del modelo de detección basado en firmas|5.2]], antes que omitirse. El **inventario completo** de la desviación, tratada como limitación del trabajo, está en [[6.1 Conclusiones|6.1]].
...
Una cifra publicada sobre NSL-KDD entra en la comparación **solo si** cumple los siete criterios C1-C7. **Su enunciado completo, su respaldo bibliográfico y la auditoría de este trabajo contra ellos tienen su sede única en [[5.0 Protocolo de evaluación#5.0.6 El mismo filtro, aplicado a uno mismo|5.0.6]]**, y no se repiten aquí; lo que sigue es únicamente su aplicación a la literatura revisada. Baste retener que ninguno de los siete es una preferencia de este trabajo: todos se apoyan en requisitos ya documentados en la literatura metodológica, tal como se detalla en 5.0.6.
...
2. **La reducción del FPR es la primera prioridad de mejora, por delante de ganar recall.** El compromiso es explícito en el marco operativo de referencia: afinar un IDPS consiste en cambiar falsos negativos por falsos positivos [25]; y la conclusión clásica de la falacia de la tasa base es que el factor limitante de la detección de intrusiones es la tasa de falsos positivos, no la sensibilidad [24]. Esta línea se recoge en [[6.2 Líneas futuras]].
...
El barrido de diez semillas obliga además a matizar una decisión tomada en el capítulo 4. La elección de estrategia de balanceo de [[4.3 Preprocesamiento de los datasets|4.3.4]] **no es constante entre semillas**, y así se declara — no se reabre el experimento ni se mueve ninguna cifra publicada, porque el hallazgo no descubre que la decisión estuviera mal: **confirma que ese eje nunca estuvo establecido**. Tres precisiones, en este orden:

- **El «no constante» afecta únicamente al eje SMOTE frente a `class_weight`**, que en el código solo aplica a **DecisionTree y RandomForest** (los otros dos algoritmos no admiten `class_weight` en scikit-learn).
- **El eje SMOTE frente a *nada*, en KNN e HistGradientBoosting, sí está establecido**: SMOTE gana en las cuatro celdas, y en HistGradientBoosting con una holgura muy amplia. Sin esta acotación se entendería que todo el apartado 4.3.4 es arbitrario, y no lo es.
- **Los márgenes del primer eje están dentro del ruido**: en el caso de RandomForest, la decisión se resuelve por un margen varias veces menor que la desviación típica **interna de la propia corrida publicada** —la que mide la dispersión entre los **cinco *folds*** de la validación cruzada con **semilla 42**, y no la dispersión entre las diez semillas del barrido, que es la que se discute en el párrafo anterior—. Ambas magnitudes apuntan en la misma dirección, pero son medidas distintas y no deben confundirse; la banda por *folds* es la que aparece junto a cada celda en [[5.2 Resultados del modelo de detección basado en firmas|5.2]]. Con ese margen, la elección es un **desempate arbitrario**, y así se escribe.

La causa mecánica es directa y está en el código: cambiar la semilla cambia el particionado de `StratifiedKFold`, de modo que tanto el ganador del mini-experimento de balanceo como la configuración elegida por el *grid* pueden cambiar con ella. **El experimento de balanceo tiene su sede en [[4.3 Preprocesamiento de los datasets|4.3.4]]**, que tabula las **ocho celdas de la variante de 54 características** y describe la de 122 en prosa, redondeada; el detalle numérico completo de **ambas variantes**, con su `±` por *folds*, está tabulado en [[5.2 Resultados del modelo de detección basado en firmas|5.2]] dentro de su lectura de resultados. Aquí no se repite ninguna de esas cifras. Conviene precisar qué contiene cada eje en esa lectura:

| Eje | Qué publica `5.2` | Con desviación |
|---|---|---|
| SMOTE vs `class_weight` (DecisionTree, RandomForest) | **4 celdas** de la corrida de la semilla 42 (2 variantes × 2 algoritmos), con ganador, `f1_macro` CV de cada opción y margen | Sí: el `±` es la desviación entre los **5 *folds*** de esa corrida |
| SMOTE vs *nada* (KNN, HistGradientBoosting) | **4 celdas** de la corrida de la semilla 42 (2 variantes × 2 algoritmos), KNN incluido | Sí: el `±` es la desviación entre los **5 *folds*** de esa corrida |

A ellas se añaden los **recuentos pareados sobre las diez semillas** de `Resultados/comparaciones_pareadas.csv`, uno por eje y sobre denominadores idénticos de **40 pares**: 23 a 17 a favor de SMOTE en el primer eje, y **40 de 40** para SMOTE en el segundo.

> [!note] La configuración efectivamente publicada es idéntica en las dos variantes de características: DecisionTree con `class_weight`; RandomForest, KNN e HistGradientBoosting con SMOTE.
...
- **El RandomForest está sobre-confiado ante lo desconocido**: de los 0-day que la etapa 1 sí detecta, solo el 13,4 % se enruta a `unknown`; el resto recibe con confianza alta una etiqueta conocida errónea. **Ese 13,4 % no es una tasa de detección fallida**: por la decisión **P-5**, `unknown` es una alarma y la marca de sospecha de la etapa 1 no se revierte, de modo que los **2.504 0-day restantes siguen estando alarmados**, solo que con la categoría equivocada. Lo que la cifra mide es el acierto al **nombrar** lo desconocido, no la cobertura de detección —que se reporta aparte, en el recall 0-day de 0,771—; el desarrollo está en [[5.3 Resultados del sistema híbrido|5.3]]. Superarlo exige **reconocimiento de conjunto abierto (*open-set*)** en lugar de un umbral sobre `predict_proba`.
