...
> [!info] Trazabilidad
> `Resultados\metricas_baseline.csv` y `Resultados\metricas_baseline_0day.csv` (filas `set_features=54`, excluyendo `tipo_0day='__global__'` salvo para el agregado), `Resultados\metricas_hibrido.csv`, `Resultados\metricas_hibrido_0day.csv`; figuras `Resultados\figuras\hibrido_cm_54.png` y `baseline_cm_54.png`. Los tres caveats que condicionan la lectura de estas cifras quedan enunciados en la propia memoria: **H-5** (la evaluación binaria del híbrido coincide con la de la etapa 1 por construcción) en el callout *Estas cifras coinciden exactamente con las de la etapa 1*, en el primer apartado de esta sección; **H-6** (no se reporta un F1 global único, porque el f1_macro a 5 clases cuenta como error los 0-day enrutados a `unknown`) en el callout *Por qué no se reporta un "F1 global" único*; y **P-2** (el recall 0-day se reporta siempre junto a su FPR, porque los dos sistemas no operan al mismo punto) en [[5.1 Resultados del modelo de detección de anomalías]] y en [[5.4 Conclusiones del capítulo]].
