...
> [!todo] Figuras de la medición (B) pendientes de copiar a `assets/`
> `validacion.py` genera además `validacion_drift_ks_d2_normales.png` (ranking de (B)) y `validacion_drift_ks_comparativa.png` (A frente a B), con sus homólogas `*_sin_seleccion` para la variante de 122. Están en `Resultados\figuras\` pero **aún no se han copiado a `Obsidian_TFG_Vault\assets\`**, por lo que no se pueden embeber todavía.
...
> [!note] Fuente numérica única
> Por decisión del 2026-08-11, las cifras del KS se publican **una sola vez, aquí en §4.2.1**. [[5.1 Resultados del modelo de detección de anomalías]] las usa **por referencia** y no las reproduce.
...
> [!info] Trazabilidad
> **Figuras:** `Resultados\figuras\eda_distribuciones_divisiones.png` (EDA, `program.py`) y `validacion_distribucion_clases.png` · `validacion_discriminantes_d1_vs_d3.png` · `validacion_drift_ks.png` · `validacion_outliers_iqr.png` (`validacion.py`), copiadas en `assets\`.
>
