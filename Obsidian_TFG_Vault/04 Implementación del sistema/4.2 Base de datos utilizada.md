...
> **4.2.1 (análisis previo)**: volcado de EDA/validación en `borrador` (Claude, 2026-07-16). **Parte teórica de 4.2** (origen del dataset, 41 features, por qué NSL-KDD): borrador redactado por Claude el 2026-07-21 (reparto de teoría cedido por acuerdo de esa fecha), cuyos marcadores `[CITA:...]` se convirtieron a `[n]` de [[Bibliografía]] en dos pases: la mayoría en el pase en serie del **2026-08-15**, y los dos de Tavallaee et al. 2009 → `[23]`, que sobrevivieron a aquel pase, el **2026-08-16**. Queda **un** marcador `[CITA: …]` vivo, el de Goldschmidt y Chudá en el bloque «Por qué se eligió NSL-KDD», pendiente del pase en serie del `researcher`.
...
> Bloques «Origen» y «Características» redactados por Claude (2026-07-21) como borrador factual; sobre el bloque «Por qué se eligió NSL-KDD» rige la reserva de revisión que se detalla más abajo, tras las cinco razones. Referencias ya numeradas contra [[Bibliografía]]: `[22]`, `[23]`, `[31]`, `[58]` y `[59]`. **Queda un marcador `[CITA: …]` pendiente de conversión**: el de Goldschmidt y Chudá (2025), citado en la razón 4 del bloque «Por qué se eligió NSL-KDD», obra que aún no está dada de alta en [[Bibliografía]]; su alta y numeración corresponden al pase en serie del `researcher`.
...
4. **Partición train/test predefinida.** Es una propiedad menos frecuente de lo que parece: solo el **23 %** de los datasets NIDS traen una partición train/test predefinida —el **16 %** si se cuentan solo los publicados desde 2020—, y su ausencia queda descrita como «*prone to biased evaluations and selective reporting*» [CITA: Goldschmidt y Chudá 2025, §5.4]. NSL-KDD sí la trae, con `KDDTrain+`/`KDDTest+` como ficheros oficiales, y este trabajo la respeta sin re-particionar (véase el apartado «Las tres particiones oficiales» más arriba): es lo que evita el sesgo de evaluación y el reporte selectivo que señalan Goldschmidt y Chudá.
...
- **El test no replica el reparto del entrenamiento.** Aquí conviene declarar **sobre qué denominador** se da cada porcentaje, porque D3 contiene *solo* ataques mientras que D2 mezcla normales y ataques, y el mismo recuento cambia de cifra según con qué se divida:
  ...
  > Conviene precisar el origen: ese reparto **no lo emite hoy ningún script** en un artefacto de
  > `Resultados\` —los soportes por clase de `metricas_firmas.csv` corresponden a la partición
  > interna de evaluación de la etapa de firmas, no a D3 entero—, sino que se obtiene por
  > **recuento directo sobre el CSV de etiquetas de D3**
  > (`specialized_nsl_kdd_processed_y_category_D3_known_attacks_for_signatures.csv`), que es
  > salida intermedia del pipeline y no está versionada. La magnitud está establecida; lo que no
  > procede es citarla como leída de una tabla de métricas publicada.
