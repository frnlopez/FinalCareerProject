...
Que exista desplazamiento en (A) no es un problema, sino **lo esperado y deseable**: D2 mezcla ataques y tipos nunca vistos, y ese cambio de distribución es precisamente lo que el detector de anomalías debe captar. Lo relevante para el diseño es que **(B) también es significativo en 25 de las 54 características**: incluso el tráfico *legítimo* de test se distribuye de forma distinta al de entrenamiento. Es **esta** medición —y no (A)— la que puede explicar que la tasa de falsos positivos real del detector sobre D2 (~8–10 %) supere la de diseño (~5 % fijada por el umbral percentil 95 sobre D1_val): los falsos positivos los generan **solo las filas normales**, de modo que un desplazamiento medido contra un D2 mayoritariamente compuesto por ataques no sostendría esa conclusión. Se discute en [[5.1 Resultados del modelo de detección de anomalías]].
...
>
> **Procedencia:** ambos informes llevan en su cabecera el `commit` y la `fecha` de la corrida, estampados automáticamente por `config.commit_actual()` (no se anclan a mano). El sello de esta corrida es **`fc1c6b4-sucio`** —el árbol de trabajo no estaba limpio al ejecutarla—, **pendiente de re-anclaje al commit de cierre**; se cita tal cual, sin sustituirlo por ningún hash.
