...
D1 y D3 particionan el train oficial (125.973 registros) sin solaparse; D2 es el test oficial íntegro, con 9.711 conexiones normales y 12.833 ataques. El resultado de esta división se ilustra en la **Figura 4.8**.

![[eda_distribuciones_divisiones.png]]

Figura 4.8. Panel de **nueve gráficos** del análisis exploratorio sobre la división D1/D2/D3: reparto de clases por subconjunto, comparación de tamaños, ataques predominantes y cobertura de tipos entre entrenamiento y test.
...
La proporción entre la clase mayoritaria y la minoritaria es ≈ 883:1. Este desbalance es el que motiva el mini-experimento de balanceo de [[#4.3.4 Equilibrio y balanceo de muestras|4.3.4]] y la elección sistemática de **f1_macro** (que pondera las cuatro clases por igual) como métrica de selección de modelos: con 52 muestras de `u2r`, un clasificador que ignorase por completo esa clase apenas perdería una milésima de accuracy. Ambas propiedades —pureza de los splits de entrenamiento y desbalance de D3— se aprecian en la **Figura 4.9**.

![[validacion_distribucion_clases.png]]

Figura 4.9. Reparto de clases por subconjunto emitido por `validacion.py`: muestra la pureza de D1 y D3 y el desbalance entre las cuatro categorías de ataque de D3. El panel de D1 dibuja una barra única rotulada `normal` con el total de sus instancias, de modo que representa la pureza **por construcción del split**; la confirmación cuantitativa corresponde al bloque de integridad de `Resultados\specialized_nsl_kdd_validation_report.txt`.

> [!todo] Figuras duplicadas entre 4.2 y 4.3 — decisión pendiente
> Las Figuras 4.8 y 4.9 embeben exactamente los mismos ficheros que las Figuras 4.1 y 4.2 de
> [[4.2 Base de datos utilizada]] (`eda_distribuciones_divisiones.png` y
> `validacion_distribucion_clases.png`). Se han numerado como instancias propias para que ningún
> embebido quede sin pie, pero en el volcado al `.docx` la misma imagen aparecería dos veces con
> dos números distintos. Queda pendiente decidir cuál de las dos apariciones se conserva y si la
> otra se sustituye por una remisión cruzada; esa decisión implica tocar la prosa de dos notas y
> excede el alcance de este pase.
