...
Conviene fijar aquí, en el cuerpo del texto y no en una nota al margen, el alcance de este apartado dentro de la memoria: **este apartado es la sede única de la verificación punto por punto del preprocesado sin fuga de datos**, y la tabla anterior es esa verificación. El criterio **C5** de la rejilla de comparabilidad de [[5.0 Protocolo de evaluación]] **no es una verificación independiente ni un segundo control**: es el **asiento resumido** de este mismo resultado dentro de la rejilla, y remite a este apartado. Por tanto, cualquier revisión del preprocesado sin fuga se hace aquí; la rejilla se limita a recoger su veredicto.
...
> [!info] Trazabilidad
> `Implementacion\app\program.py` (pasos 2, 3 y 4 de `preprocess_specialized_splits` y `select_features`) · `Implementacion\PIPELINE.md` (orden del pipeline y artefactos por variante) · rejilla C1–C7 y criterio C5: [[5.0 Protocolo de evaluación]] · desviación declarada de selección sobre D2: [[#4.3.5 Selección de características|4.3.5]] § *Decisión experimental*, sede canónica de esa declaración.
