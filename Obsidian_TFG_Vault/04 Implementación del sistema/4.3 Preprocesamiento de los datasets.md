...
> [!info] Trazabilidad
> `Resultados\selected_features.txt` (cifras y listados de la selección) · `Implementacion/app/program.py` (`select_features`, umbrales 10⁻⁸ / 0,95 / 0,999) · veredicto H1 —adoptar el set de **54** características para ambas etapas—: enunciado y razonado en el § *Decisión experimental* de este mismo apartado, con cifras de `Resultados\metricas_hibrido.csv` y `Resultados\metricas_hibrido_0day.csv`.
...
> [!info] Trazabilidad
> `Implementacion/app/program.py` (pasos 2, 3 y 4 de `preprocess_specialized_splits` y `select_features`) · `Implementacion/PIPELINE.md` (orden del pipeline y artefactos por variante) · rejilla C1–C7 y asiento del criterio C5 (remite a este apartado, sede única): [[5.0 Protocolo de evaluación]] · desviación declarada de selección sobre D2: [[#4.3.5 Selección de características|4.3.5]] § *Decisión experimental*, sede canónica de esa declaración.
