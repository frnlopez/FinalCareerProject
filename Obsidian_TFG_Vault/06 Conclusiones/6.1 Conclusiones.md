...
- *Razón técnica.* La evaluación es **en diferido y sobre datos ya capturados**: no hay sensor en línea, ni prevalencia real de clases, ni analista en el bucle. Arp et al. clasifican esta situación como el pitfall **P9** [CITA: Arp et al. 2022, P9 *Lab-Only Evaluation*].
...
- *Qué se hizo en su lugar.* P9 se cubre en sus **dos mitades viables sin desplegar**:
	1. **El coste de despliegue se mide**, en lugar de omitirse: cada modelo reporta tiempo de entrenamiento, tiempo de inferencia, **latencia por flujo** y **caudal en flujos por segundo**, con el alcance declarado, en las tablas de `Resultados/`. Estas cifras llevan además su propia salvedad —el reloj de pared se tomó en una máquina no dedicada, y su dispersión mide **carga de máquina**, no el algoritmo: una misma configuración varió hasta **4,8×** entre dos corridas con calidad idéntica al bit (`Resultados/dispersion_semillas.md`)—, por lo que se citan como orden de magnitud y no como medida de rendimiento del método.
	2. **La ausencia de proyección a la prevalencia real se declara** junto a cada cifra de precisión, en lugar de dejar que el lector la extrapole.
...
- *Razón técnica.* La etiqueta `unknown` del sistema se decide con un **umbral sobre la probabilidad** que emite el clasificador de firmas. Pero ese clasificador es de **conjunto cerrado**: su salida es una distribución sobre las categorías que vio en entrenamiento, y no contiene información sobre la posibilidad de que la muestra no pertenezca a ninguna. Umbralizar esa salida **no acota el riesgo de espacio abierto**, que es la exposición del modelo a la región del espacio de características que sus datos de entrenamiento no cubren [CITA: Scheirer et al. 2013, «Toward Open Set Recognition», riesgo de espacio abierto].
...
| **P8** | *Base Rate Fallacy* | Cubierto | **Ninguna cifra de recall se publica sin su FPR**, y las clases minoritarias se reportan aparte en lugar de ocultarse en el promedio |
| **P9** | *Lab-Only Evaluation* | **Cubierto hasta donde el dataset permite** | Sin despliegue real. Se mide el **coste de despliegue** (latencia por flujo y caudal, con su salvedad de máquina) y se declara que la precisión **no está proyectada** a una prevalencia operativa — límite 2 de este apartado |
| **P10** | *Inappropriate Threat Model* | **Cubierto hasta donde el dataset permite** | Sin adversario adaptativo. El modelo de amenaza se **escribe** en [[3.1 Requisitos del sistema]] y se argumenta, con su fuente, por qué un experimento adversario sobre características agregadas sería irrealizable en el espacio del problema — límite 3 |
...
> Este apartado remite **por nombre de sección** a contenidos que se desarrollan en otros capítulos —el modelo de amenaza explícito en [[3.1 Requisitos del sistema]], la lectura de las bandas de dispersión en [[5.2 Resultados del modelo de detección basado en firmas]] y [[5.4 Conclusiones del capítulo]], y los tres peldaños de continuación en [[6.2 Líneas futuras]]—. Las remisiones indican **dónde** se trata cada cosa; no anticipan su contenido ni ninguna cifra que no esté ya publicada en `Resultados/`.
