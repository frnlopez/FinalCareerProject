...
**El precio del orden elegido, declarado.** La cascada no sale gratis: anteponer un detector de anomalías con un umbral fijado por diseño introduce falsas alarmas que un clasificador supervisado no comete. Sobre D2 completo, el híbrido registra un **FPR de 10,2 % en la variante de 54 características y de 8,5 % en la de 122** (`Resultados\metricas_hibrido.csv`, columna `bin_fpr`), frente al **2,7 %** del baseline RandomForest monolítico (`Resultados\metricas_baseline.csv`). El diseño se defiende por lo que compra con ese margen —la detección de tipos ausentes del entrenamiento—, contraste que se cuantifica en [[5.3 Resultados del sistema híbrido]]; enunciar la ventaja sin este coste sería incompleto.
