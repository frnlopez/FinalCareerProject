...
A esa evidencia externa se suma una comprobación interna con los datos del propio trabajo: el conjunto por *boosting* que sí forma parte del comparador, HistGradientBoosting, **no supera** a RandomForest en la variante de 54 características (`f1_macro` **0,804** frente a **0,822**, `Resultados/metricas_firmas.csv`). Si el *boosting* de gradiente moderno de la propia `scikit-learn` no despega en este problema, la expectativa de que lo haga una arquitectura de atención con un espacio de hiperparámetros mucho mayor es baja.
