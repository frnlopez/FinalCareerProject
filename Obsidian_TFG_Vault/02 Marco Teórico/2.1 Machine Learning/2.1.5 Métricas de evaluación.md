...
Elegir un algoritmo y entrenarlo no basta: hace falta una forma de decir, con precisión, si el resultado es bueno. Esa forma es la **métrica**, y su elección no es un detalle técnico menor —una métrica mal elegida puede hacer pasar por excelente un modelo inútil, como se verá en 2.1.5.3 con la exactitud bajo clases desequilibradas—. Esta sección presenta las métricas de clasificación, regresión y *clustering* que se emplean a lo largo de la memoria, en su formulación general. Su aplicación concreta a las cuatro tablas de resultados del sistema —con las cifras que arroja cada una— se recoge en [[A.2 Métricas de desempeño]], a la que esta sección remite en lugar de anticipar ningún número.

> [!note] Reparto de roles con el Apéndice A.2
> Esta sección es **marco teórico**: qué es cada métrica, cómo se define y por qué importa para un problema como la detección de intrusiones. [[A.2 Métricas de desempeño]] es **material de consulta**: el mismo catálogo, aplicado a las cuatro tablas de resultados de este sistema (matriz de confusión binaria y multiclase, umbral por percentil del detector de anomalías, FPR nominal frente a FPR medido). La duplicación entre ambas es deliberada: aquí se explica el porqué; allí se documenta el cómo, con las cifras concretas del proyecto.
...
De ahí se sigue una consecuencia práctica que esta memoria aplica de forma sistemática: **una cifra de recall aislada, sin su FPR asociada, no describe un detector**. Un recall del 95 % suena excelente hasta que se pregunta a qué FPR se alcanzó: si ese mismo umbral marca como ataque el 40 % del tráfico normal, el sistema es inservible en producción, por más alto que sea su recall, porque la avalancha de falsas alarmas resultante hace inviable la respuesta del analista. El recall y el FPR son, en rigor, dos coordenadas del **mismo punto de una curva** —la que describe 2.1.5.5—, no dos métricas independientes que puedan juzgarse por separado. Publicar solo una de las dos equivale a describir un punto del plano dando una única coordenada.
