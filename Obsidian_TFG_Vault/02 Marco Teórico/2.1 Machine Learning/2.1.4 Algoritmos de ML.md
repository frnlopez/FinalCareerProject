...
El algoritmo de los k vecinos más cercanos (*k-Nearest Neighbors*, k-NN) es el ejemplo más conocido de **aprendizaje perezoso** (*lazy learning*): en lugar de estimar una función general durante el entrenamiento, se limita a almacenar todos los ejemplos de entrenamiento y pospone todo el cálculo al momento de clasificar [CITA: Géron, cap. 3].
...
La noción de «cercanía» depende de una métrica de distancia elegida según el tipo de dato: la distancia euclidiana para variables continuas y la distancia de Hamming para variables discretas son las más habituales [CITA: Géron, cap. 3].
...
> [!warning] Atribución pendiente de verificar
> Los dos párrafos anteriores atribuyen la descripción de Isolation Forest a `[CITA: Molnar, cap. 8]`, obra centrada en interpretabilidad de modelos y que **no es la fuente original del algoritmo** (Liu, Ting y Zhou, 2008). El `researcher` debe verificar esta atribución —y decidir si procede reasignarla a la fuente primaria— **antes** de convertir el marcador a un `[n]`.
> Se añade además que la **edición de la obra de Molnar no está fijada** (la web sirve hoy la 3.ª; la literatura cita la 2.ª de 2022), de modo que el localizador `cap. 8` queda condicionado a esa decisión, reservada a Francisco.

### 2.1.4.2.2 One-Class SVM

One-Class SVM adapta el principio de las máquinas de vectores de soporte (*Support Vector Machines*) al aprendizaje con una sola clase. En lugar de buscar un hiperplano que separe dos clases, busca la **frontera** —en el espacio transformado por un núcleo (*kernel*), típicamente el radial (RBF)— que envuelve la región donde se concentra la clase normal, dejando fuera de ella la menor fracción posible de puntos de entrenamiento compatible con un margen de tolerancia fijado de antemano [CITA: Géron, cap. 5]. Cualquier ejemplo nuevo que caiga fuera de esa frontera se marca como anómalo.
