...
Lo que este trabajo usa de esa familia es un componente concreto: un MLP empleado como **autoencoder** —se entrena para reconstruir su propia entrada, y el error de reconstrucción sirve de puntuación de anomalía—, que es el detector seleccionado para la etapa 1 (véase [[4.4 Entrenamiento del modelo de detección de anomalías|4.4]]). **El desarrollo completo —neurona y perceptrón, funciones de activación, descenso de gradiente y retropropagación— está en [[A.3 Ficha del sistema|A.3]] (§A.3.10.3) y no se reproduce aquí.** La frontera entre las redes neuronales y el *deep learning* la fija [[2.3.1 IA, ML y Deep Learning|2.3.1]].
