...
> **El inventario de las tres, con su criterio y su registro, se desarrolla en [[4.3 Preprocesamiento de los datasets|4.3.5]] y en [[6.1 Conclusiones|6.1]] (§6.1.3), y se tabula en [[A.3 Ficha del sistema|A.3]] (§A.3.7, «Límite de protocolo»); aquí no se reproduce. El fundamento de la partición y el desarrollo de la línea roja del test se recogen en §A.3.11.2.**
...
> La referencia a Arp et al. [18] se usa en este apartado y en [[#2.1.6.6 Métricas ante desbalance: por qué el *accuracy* miente|2.1.6.6]] **solo por la denominación y la numeración de los *pitfalls*** —P3 *Data Snooping* y la medida de rendimiento inapropiada—, tomadas de su enumeración. **No se toma de ella ninguna cifra**, y en particular **no se apoya ningún dato de su Figura 1**, cuyos porcentajes de prevalencia no se han podido leer del texto completo por falta de acceso institucional. Queda pendiente comprobar contra el original la numeración y la redacción exacta de esos dos defectos; el argumento de ambos apartados se sostiene íntegro si la referencia se retira.
...
**Qué es** la fuga de información, **cómo se manifiesta** y por qué cada una de las tres formas contamina la medida se explican aquí, y los apartados del capítulo 4 remiten a este apartado en lugar de reexplicarlo. Cómo la evita el sistema desarrollado es materia de implementación y se documenta en [[4.3 Preprocesamiento de los datasets|4.3]]: la verificación punto por punto de dónde se ajusta cada transformación que aprende algo de los datos, en §4.3.7; el sobremuestreo **dentro de cada partición**, con el `ImbPipeline` que lo materializa, en §4.3.4; y el vocabulario *one-hot* construido como unión del entrenamiento, junto con el defecto simétrico que se corrigió, en §4.3.3. El perímetro complementario, el que cubre las **decisiones de selección de modelo** en lugar del preprocesado, es el de 2.1.6.2: son dos riesgos distintos y ninguno de los dos exime del otro.
...
**Cómo pondera cada promedio y qué efecto tiene bajo desbalance se tabula en [[2.1.5 Métricas de evaluación|2.1.5]] (§2.1.5.3) y no se reproduce aquí.** Lo que importa aquí es la consecuencia: `f1_macro` es el criterio de este trabajo precisamente porque **no deja que una clase minoritaria mal resuelta pase desapercibida**.
