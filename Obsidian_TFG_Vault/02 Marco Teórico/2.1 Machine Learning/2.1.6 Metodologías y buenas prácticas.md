...
El trabajo se ordena según **CRISP-DM** (*Cross-Industry Standard Process for Data Mining*), marco iterativo de seis fases —comprensión del problema, comprensión de los datos, preparación, modelado, evaluación y despliegue [11]—, adoptado como marco de ordenación del relato y no como corsé. Su mapeo a este TFG es directo, con una salvedad que se declara: la fase de **despliegue queda fuera del alcance** ([[6.1 Conclusiones]]), porque el sistema es un prototipo de investigación y presentar resultados de laboratorio como capacidades operativas es precisamente una de las críticas recurrentes al aprendizaje automático aplicado a la seguridad. Las cuatro fases centrales se materializan, respectivamente, en el EDA y la validación de splits (`program.py`, `validacion.py`), el preprocesado ([[4.3 Preprocesamiento de los datasets]]), las dos etapas por separado ([[4.4 Entrenamiento del modelo de detección de anomalías]] y [[4.5 Entrenamiento del modelo de detección basado en firmas]]) y la evaluación única de extremo a extremo sobre D2 ([[5.3 Resultados del sistema híbrido]]).
...
> Quedan fuera del perímetro tres decisiones que se declaran de forma expresa: **el detector de la etapa 1, el clasificador de la etapa 2 y el set de características (54 frente a 122) se eligieron con métricas medidas sobre D2**.
>
> | Etapa | Elegido | Criterio con el que se eligió | Dónde consta |
> |---|---|---|---|
> | 1 — anomalías | **Autoencoder-MLP** | AUC-ROC y F1 **sobre D2**, en 54 y 122 características | Decisión **H-2**, `resumen-de-decisiones.md`; [[5.1 Resultados del modelo de detección de anomalías]] |
> | 2 — firmas | **RandomForest** | `f1_macro` medido sobre D2 | Decisión **H-3**, `resumen-de-decisiones.md`; [[5.2 Resultados del modelo de detección basado en firmas]] |
> | Set de características | **54** (frente a 122) | Recall 0-day por tipo + `f1_macro`, cascada AE→RF **sobre D2** (experimento H1) | Decisión **Q1/C**; [[4.3 Preprocesamiento de los datasets|4.3.5]] § «Decisión experimental: 54 frente a 122 (experimento H1, medido sobre D2)» |
>
> Las tres son **selección de modelo sobre el test**, es decir *data snooping* —el error catalogado como tal, junto con la selección sesgada de parámetros, en el inventario de errores metodológicos del aprendizaje automático aplicado a la seguridad [18]—, y las métricas publicadas son por tanto optimistas respecto a un test verdaderamente ciego. La tercera es la más estructural: no elige un modelo dentro de un espacio de características dado, sino que **define ese espacio** para todo el sistema publicado. Su magnitud no se estima —haría falta un conjunto de decisión independiente de D2—: se acota el alcance a estas tres decisiones y se advierte al lector de los capítulos 5 y 6.
>
> **Sedes canónicas**: el reparto objeto por objeto —qué se fija en [[6.1 Conclusiones]], qué en 4.3.5 y qué en la ficha— está tabulado en [[A.3 Ficha del sistema]] § A.3.11.2 y **no se reproduce aquí**. Lo que esta sección conserva es solo la lectura conceptual: usa el inventario como ejemplo de qué es el *data snooping* y de por qué el perímetro anterior no basta para impedirlo. La corrección —repetir las tres comparaciones sobre un conjunto de decisión independiente de D2— queda como línea futura en [[6.2 Líneas futuras]].
...
> Esta sección cierra el bloque de aprendizaje automático del marco teórico: se apoya en [[2.1.3 Ciclo de vida de un proyecto ML]] y en [[2.1.5 Métricas de evaluación]], y sirve de referencia metodológica a [[4.3 Preprocesamiento de los datasets]], [[4.5 Entrenamiento del modelo de detección basado en firmas]] y [[5.4 Conclusiones del capítulo]].
