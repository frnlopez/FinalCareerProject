...
El aprendizaje por refuerzo es un tercer paradigma, de uso menos extendido en este trabajo pero conceptualmente distinto de los dos anteriores. Consiste en aprender **qué hacer**, es decir, cómo relacionar situaciones (estados) con acciones, con el fin de maximizar una señal de recompensa numérica acumulada a lo largo del tiempo. A diferencia del aprendizaje supervisado, al agente no se le indica qué acción es correcta en cada paso: debe **descubrir** por sí mismo, mediante prueba y error, qué secuencia de acciones produce la mayor recompensa.

Este trabajo no emplea aprendizaje por refuerzo — ni la etapa de anomalías ni la de firmas requieren un agente que actúe sobre un entorno y reciba recompensa demorada; ambas son tareas de detección sobre datos estáticos ya recogidos. Se incluye en esta sección por completitud taxonómica, y porque delimita por contraste qué **no** es este proyecto: no hay entorno, no hay acción, no hay política que optimizar.
...
No se emplea aprendizaje por refuerzo ni aprendizaje no supervisado en sentido estricto en ningún componente del sistema: incluso el agrupamiento o la reducción de dimensionalidad, si se usan en el proyecto, lo hacen como herramientas auxiliares de análisis exploratorio y no como los detectores finales.
