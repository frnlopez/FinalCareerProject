...
> [!note] Dónde queda este TFG
> El sistema desarrollado se sitúa en el **anillo intermedio**: es aprendizaje automático. Contiene un único componente neuronal —un autoencoder implementado sobre `MLPRegressor` de scikit-learn— que, por su tamaño y por el papel que desempeña, **no constituye una red profunda**. La afirmación es deliberada y coherente con lo declarado en [[3.1 Requisitos del sistema]] y en [[3.4 Modelo de detección de anomalías]]; su justificación se desarrolla en 2.3.1.4.

---

## 2.3.1.2 Un recorrido histórico mínimo

La historia de la IA se recoge aquí solo en la medida en que explica el vocabulario que se acaba de fijar; no se pretende un panorama del campo [CITA: Russell y Norvig, Artificial Intelligence: A Modern Approach].

| Etapa | Hito | Qué aporta al vocabulario |
|---|---|---|
| **1956** | Conferencia de Dartmouth | Acta fundacional de la disciplina y del propio término «Inteligencia Artificial» |
| **Décadas de 1970 y 1980** | Ciclos de expectativa y retracción (los «inviernos de la IA») | Muestran que el campo ha avanzado por oleadas, con promesas que excedieron a los resultados |
| **Años 80** | Auge de los **sistemas expertos** | Cumbre del paradigma de reglas: el conocimiento del especialista, codificado a mano en una base de reglas |
| **Años 90 y 2000** | Consolidación del **ML estadístico** | El conocimiento deja de escribirse y pasa a estimarse a partir de datos |
| **Desde 2012** | Resultados de las redes convolucionales profundas en el concurso ImageNet | Da nombre y tracción al **aprendizaje profundo** como subcampo diferenciado [CITA: Krizhevsky et al. 2012, ImageNet Classification with Deep Convolutional Neural Networks] |

Interesa destacar la transición entre las dos etapas centrales, porque es exactamente la que atraviesa este trabajo en su propio dominio: **el paso del conocimiento escrito a mano al conocimiento inducido de los datos**.

---

## 2.3.1.3 Reglas escritas a mano frente a reglas aprendidas de los datos
...
> Este contraste es exactamente el que separa a los dos paradigmas de detección comparados en [[2.2.4 Detección por firmas frente a detección por anomalías]]: un IDS por firmas clásico —Snort es el ejemplo canónico— **es** un sistema de reglas escritas a mano.
>
> Y aquí aparece el punto que hace interesante la etapa 2 de este trabajo: en lugar de escribir las firmas, se **aprenden** de ataques etiquetados y después se **devuelven en formato legible** mediante `export_text` sobre el árbol de decisión, volcadas en `firmas_reglas_54.txt` y comentadas en [[4.5 Entrenamiento del modelo de detección basado en firmas]]. El sistema no elige entre los dos paradigmas: induce las reglas con ML y las entrega con la explicabilidad propia del enfoque clásico.

---

## 2.3.1.4 Por qué este trabajo usa ML clásico y no aprendizaje profundo
...
1. **Escala de la arquitectura.** Las configuraciones candidatas son perceptrones multicapa de tres capas ocultas y anchura decreciente-creciente, del orden de decenas de neuronas por capa (`anomalias.py:86-88`) — muy lejos de las jerarquías profundas características del DL.
...
## 2.3.1.5 Situación del trabajo y transición
