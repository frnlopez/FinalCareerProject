...
La cadena tiene una lectura incómoda y necesaria: **un detector con métricas de laboratorio excelentes puede resultar inutilizable, e incluso contraproducente, en un centro de operaciones real**. La métrica agregada —una AUC alta, una exactitud alta— no mide utilidad operativa, porque no incorpora ni la prevalencia real ni el coste del triaje. De ahí se sigue el criterio metodológico que este trabajo adopta y que se detalla en [[2.1.5 Métricas de evaluación]]: **ninguna cifra de recall se reporta sin su tasa de falsos positivos**, y el punto de operación se declara siempre de forma explícita.

> [!note] Conexión con el sistema implementado
> En este trabajo la tasa de falsos positivos no es un resultado accidental sino una **variable de diseño declarada**: el umbral del detector de anomalías se fija en el **percentil 95** de la puntuación sobre el subconjunto de validación de D1, lo que equivale a aceptar por construcción del orden de un 5 % de falsas alarmas sobre tráfico legítimo a cambio de sensibilidad ([[3.4 Modelo de detección de anomalías]]). El contraste entre esa tasa asumida por diseño y la efectivamente observada sobre el conjunto de evaluación es material del capítulo 5 y no se anticipa aquí.
...
Esta preocupación no es nueva ni exclusiva de Arp et al. Sommer y Paxson ya argumentaron que la detección de intrusiones presenta rasgos que la hacen especialmente hostil para el aprendizaje automático frente a otros dominios —variabilidad extrema de lo que se considera «normal», coste asimétrico de los errores y dificultad de obtener datos etiquetados representativos—, y señalaron además el ***semantic gap***: el fallo característico de un detector de anomalías no es dejar de detectar, sino producir una salida que el operador **no puede convertir en una acción** [CITA: Sommer y Paxson 2010, Outside the Closed World: On Using Machine Learning for Network Intrusion Detection]. Las dos advertencias apuntan al mismo sitio: la métrica de laboratorio y la utilidad operativa son magnitudes distintas.

> [!note] Alcance de este trabajo
> El sistema desarrollado se evalúa **en diferido y sobre datos ya capturados** ([[2.2.3 Taxonomía de los IDS]]), de modo que P9 **no queda cubierto por un despliegue real**, cosa que excede el alcance de un TFG. Lo que sí es exigible, y es la postura que adopta esta memoria, es **declarar el límite y cubrir la parte viable**: proyectar los resultados a una prevalencia realista para mostrar cuál sería la precisión operativa esperable. Esa proyección es un resultado y se presenta en el capítulo 5, no aquí.
...
- **Triaje.** El analista debe decidir en minutos si la alerta merece investigación. Una salida que dice «anómalo, puntuación 0,87» no orienta esa decisión; una que dice «coincide con el patrón de conexiones semiabiertas hacia un mismo puerto» sí.
