...
El tratamiento es deliberadamente de marco teórico. Aquí se enuncian los límites **como categoría conceptual**, con su razón técnica y su respaldo bibliográfico; el inventario de los límites concretos de este trabajo, con sus cifras, corresponde a la discusión de resultados y a [[6.1 Conclusiones]], y el modelo de amenaza específico del sistema desarrollado se fija en [[3.1 Requisitos del sistema]]. Esta separación no es cosmética: un límite declarado en el marco teórico es un rasgo del **problema**; un límite declarado en las conclusiones es un rasgo de **esta solución**, y confundirlos debilita ambos.

El hilo conductor lo aporta la revisión de Arp et al., que sistematiza **diez errores metodológicos recurrentes** (*pitfalls*) en la aplicación de aprendizaje automático a seguridad informática, agrupados por fase del flujo de trabajo. Su hallazgo, sobre una revisión de treinta artículos publicados en los cuatro congresos de referencia del área durante la década anterior, es que *«each paper suffers from at least three pitfalls»* [CITA: Arp et al. 2022, Dos and Don'ts of Machine Learning in Computer Security]. Dos de esos diez errores —**P9, *Lab-Only Evaluation***, y **P10, *Inappropriate Threat Model***— son precisamente límites de la **evaluación experimental** como método, y por eso se desarrollan aquí y no en el capítulo de resultados.

---

## 2.3.3.1 El coste de los falsos positivos en operación

El primer límite no es del algoritmo sino de la **relación entre el algoritmo y la organización que lo opera**. En [[2.2.4 Detección por firmas frente a detección por anomalías]] se expone la **falacia de la tasa base** aplicada a la detección de intrusiones: dado que el tráfico legítimo supera al malicioso en varios órdenes de magnitud, una tasa de falsos positivos aparentemente pequeña produce en términos absolutos muchas más alertas erróneas que correctas, de modo que la precisión —y no el recall— es el factor limitante del sistema [CITA: Axelsson 1999, base-rate fallacy].

Lo que interesa añadir aquí es la **consecuencia humana** de ese resultado estadístico, que es donde el problema deja de ser técnico:

| Eslabón | Mecanismo | Efecto sobre el sistema socio-técnico |
|---|---|---|
| Volumen absoluto de alertas | Cada alarma consume tiempo de analista para su triaje | Coste económico directo, proporcional al volumen |
| Saturación del equipo | La cola de alertas crece más rápido de lo que se vacía | Se prioriza por heurísticas informales, no por riesgo |
| **Fatiga de alertas** | Desensibilización: la alerta deja de leerse como señal | Aumenta la probabilidad de descartar un verdadero positivo |
| Pérdida de confianza | Se silencian reglas o se elevan umbrales sin análisis | El sistema pierde cobertura de forma no documentada |
| Abandono efectivo | El detector sigue encendido pero ya no se atiende | Coste mantenido, beneficio nulo |

La cadena tiene una lectura incómoda y necesaria: **un detector con métricas de laboratorio excelentes puede resultar inutilizable, e incluso contraproducente, en un centro de operaciones real**. La métrica agregada —una AUC alta, una exactitud alta— no mide utilidad operativa, porque no incorpora ni la prevalencia real ni el coste del triaje. De ahí se sigue el criterio metodológico que este trabajo adopta: **ninguna cifra de recall se reporta sin su tasa de falsos positivos**, y el punto de operación se declara siempre de forma explícita. La **tasa de falsos positivos** (*false positive rate*, FPR) es la proporción de conexiones legítimas que el detector señala como ataque, calculada **dentro** de la clase normal; el **recall** es la proporción de ataques que sí detecta, calculada dentro de la clase de ataque. Se calculan, por tanto, sobre poblaciones distintas —no son una el complemento de la otra— y ninguna de las dos es interpretable sola: subir un umbral mejora una a costa de la otra, de modo que una cifra de recall sin su FPR no describe un detector, sino un punto arbitrario de una curva.

> [!todo] Ampliar cuando se redacte [[2.1.5 Métricas de evaluación]]
> La definición formal de la tasa de falsos positivos y del recall a partir de la matriz de confusión corresponde a esa sección, hoy en borrador. Aquí se da la glosa mínima para que el argumento se siga sin ella.

> [!note] Conexión con el sistema implementado
> En este trabajo la tasa de falsos positivos no es un resultado accidental sino una **variable de diseño declarada**: el umbral del detector de anomalías se fija en el **percentil 95** de la puntuación sobre el subconjunto de validación de D1, lo que equivale a aceptar por construcción del orden de un 5 % de falsas alarmas **sobre el tráfico normal de `D1_val`** a cambio de sensibilidad ([[3.4 Modelo de detección de anomalías]]). El contraste entre esa tasa asumida por diseño y la efectivamente observada sobre el conjunto de evaluación es material del capítulo 5 y no se anticipa aquí.

---

## 2.3.3.2 Evaluación de laboratorio frente a evaluación operativa (P9)

El apartado anterior describe un efecto; este describe **por qué un experimento no lo captura**. El pitfall P9 de Arp et al., *Lab-Only Evaluation*, enuncia que un sistema de seguridad evaluado exclusivamente en condiciones de laboratorio **no ha demostrado nada sobre su comportamiento en producción**, por buenos que sean sus números [CITA: Arp et al. 2022, Dos and Don'ts of Machine Learning in Computer Security]. La distancia entre ambos escenarios no es de grado, sino de naturaleza, y se descompone en al menos cuatro factores independientes:
...
Esta preocupación no es nueva ni exclusiva de Arp et al. Sommer y Paxson ya argumentaron que la detección de intrusiones presenta rasgos que la hacen especialmente hostil para el aprendizaje automático frente a otros dominios —variabilidad extrema de lo que se considera «normal», coste asimétrico de los errores y dificultad de obtener datos etiquetados representativos—, y señalaron además el ***semantic gap***, definido en [[2.1.6 Metodologías y buenas prácticas]] [CITA: Sommer y Paxson 2010, Outside the Closed World: On Using Machine Learning for Network Intrusion Detection]. El matiz pertinente **para este apartado** es que ese desajuste es justamente uno de los que una evaluación de laboratorio **no puede medir**: al no haber analista en el bucle, ninguna métrica del experimento registra si la salida era convertible en una acción. Las dos advertencias apuntan al mismo sitio: la métrica de laboratorio y la utilidad operativa son magnitudes distintas.
...
El segundo límite de método es el pitfall P10, *Inappropriate Threat Model*: **un sistema de seguridad solo está definido cuando está definido el adversario frente al cual pretende protegerlo** [CITA: Arp et al. 2022, Dos and Don'ts of Machine Learning in Computer Security]. Sin esa definición, afirmar que un detector «funciona» carece de contenido verificable, porque no se ha dicho contra qué.
...
Se ha argumentado que, en decisiones de alto riesgo, la práctica de explicar *a posteriori* un modelo opaco es preferible sustituirla por el uso de un modelo interpretable desde el diseño, precisamente porque una explicación aproximada puede diferir de la lógica efectiva del sistema [CITA: Rudin 2019, Stop explaining black box machine learning models for high stakes decisions]; la sistematización del campo y de sus técnicas está recogida en la literatura de referencia sobre aprendizaje automático interpretable [CITA: Molnar, Interpretable Machine Learning].

> [!note] Conexión con el sistema implementado
> Este trabajo ejerce la explicabilidad, no solo la cita. Del árbol de decisión entrenado en la etapa de firmas se extraen **reglas legibles** —condiciones sobre banderas de conexión y tasas de error que conducen a una categoría de ataque— volcadas a un fichero de texto y comentadas en [[4.5 Entrenamiento del modelo de detección basado en firmas]]. Un analista puede leerlas, contrastarlas con su conocimiento del dominio y discutirlas, cosa impracticable con un *ensemble* de cientos de árboles. Es el mismo paralelismo con las firmas escritas a mano que se establece en [[2.2.4 Detección por firmas frente a detección por anomalías]].

---

## 2.3.3.5 Privacidad del tráfico de red

Monitorizar una red implica tratar datos que describen la actividad de personas: con quién se comunican, qué servicios usan, en qué horarios y con qué intensidad. Aun sin acceder al contenido, esos metadatos permiten inferir hábitos, relaciones y, en determinados contextos, categorías especialmente protegidas. En el marco jurídico europeo, una dirección IP asociable a una persona física identificable constituye **dato personal**, de modo que un sistema de detección de intrusiones queda dentro del ámbito del Reglamento General de Protección de Datos [CITA: Reglamento (UE) 2016/679, RGPD].
...
**Límite de sesgo y obsolescencia del corpus.** Los conjuntos de datos de detección de intrusiones se construyen en condiciones controladas y arrastran las decisiones de esa construcción: tráfico de fondo sintético, mezcla de ataques que no refleja frecuencias reales, artefactos de generación que un modelo puede aprender como atajo. A ello se suma el envejecimiento: protocolos, servicios y patrones de uso cambian a lo largo de los años, de modo que un corpus antiguo describe una red que ya no existe. Ambos efectos están documentados con detalle para la familia DARPA/KDD, primero en la crítica metodológica de la evaluación original [CITA: McHugh 2000, Testing Intrusion Detection Systems: a critique of the 1998 and 1999 DARPA IDS evaluations] y después en el análisis que motivó la construcción de NSL-KDD [CITA: Tavallaee et al. 2009, A detailed analysis of the KDD CUP 99 data set].
...
|---|---|---|
| **RGPD** — Reglamento (UE) 2016/679 | Protección de datos personales | El tráfico monitorizado contiene datos personales: obliga a base jurídica, minimización, limitación de finalidad y plazos de conservación [CITA: Reglamento (UE) 2016/679, RGPD] |
| **AI Act** — Reglamento (UE) 2024/1689 | Enfoque **basado en riesgo**: prácticas prohibidas, alto riesgo, riesgo limitado y riesgo mínimo, con obligaciones graduadas | Fija obligaciones transversales de gestión de riesgos, calidad de los datos, documentación técnica, transparencia y **supervisión humana** para los sistemas del nivel alto [CITA: Reglamento (UE) 2024/1689, Ley de IA] |
| **NIS2** — Directiva (UE) 2022/2555 | Ciberseguridad de sectores esenciales e importantes | Impone medidas de gestión de riesgos y obligaciones de **detección y notificación de incidentes**: es el contexto normativo que empuja el despliegue de sistemas como el aquí estudiado [CITA: Directiva (UE) 2022/2555, NIS2] |

> [!todo] Encaje exacto del sistema en la clasificación de riesgo del AI Act
> La determinación de si un detector de intrusiones concreto entra en la categoría de **alto riesgo** del AI Act depende de su finalidad, del sector de despliegue y de los anexos de la norma, y **exige la lectura del texto consolidado**, que no se ha realizado en esta redacción. Se enuncia por tanto el enfoque basado en riesgo y las obligaciones generales, **sin afirmar en qué categoría cae el sistema de este trabajo**. Pendiente de completar con el texto oficial en EUR-Lex.
...
> **Arp et al. (2022)** [CITA: Arp et al. 2022, Dos and Don'ts of Machine Learning in Computer Security] consta en `99 Investigación/` como **verificada solo por su resumen y metadatos**, sin acceso al texto completo, y es la fuente que vertebra esta sección: de ella proceden el **catálogo de diez errores metodológicos**, la denominación y el contenido de **P9 (*Lab-Only Evaluation*)** y **P10 (*Inappropriate Threat Model*)** —que dan título a 2.3.3.2 y 2.3.3.3— y la **cita textual** sobre la prevalencia de los errores en los artículos revisados.
>
> Lo que queda por comprobar frente al artículo original es la **numeración, la denominación exacta y el enunciado** de esos dos errores, así como la literalidad y el contexto de la cita entrecomillada. Las afirmaciones que sostienen esos apartados —que una evaluación de laboratorio no demuestra comportamiento en producción y que un sistema de seguridad no está definido sin su modelo de amenaza— **se argumentan aquí por su razón técnica y no dependen de la fuente para sostenerse**; lo que depende de ella es la atribución nominal y el respaldo bibliográfico. **No se toma de ella ninguna cifra** más allá de la frase citada.
