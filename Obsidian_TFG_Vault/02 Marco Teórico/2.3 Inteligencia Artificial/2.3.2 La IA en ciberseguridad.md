...
> [!warning] Caso de uso RETIRADO el 2026-08-18 — cita `[44]` irreconstruible
> Aquí iba un cuarto caso de uso, «Apoyo al analista en el SOC» (el valor del aprendizaje automático
> en priorizar el triaje, agrupar alertas y descartar patrones benignos, más los modelos de lenguaje
> como asistentes de investigación), apoyado en `[44]`. **`[44]` es la peor de las cinco citas
> colgantes irreconstruibles**: el texto no daba ni autor ni clase de documento. **Francisco ha
> decidido retirar la afirmación, no reetiquetarla.** El caso ya se declaraba **fuera del alcance de
> este trabajo**, así que su retirada no deja hueco argumental. La entrada `[44]` **no se borra ni se
...
> [!warning] Afirmación RETIRADA el 2026-08-18 — citas `[37]` y `[45]` irreconstruibles
> Este apartado cerraba con una afirmación agregada —«el efecto más relevante no es un ataque
> cualitativamente nuevo, sino el descenso de la barrera de entrada y el aumento de la escala de los
> ataques ya existentes»— avalada por `[37]` y `[45]`, dos **informes de agencia europea cuya edición
> concreta nunca se pudo fijar**: son dos de las cinco citas colgantes irreconstruibles. Es una
> afirmación **de tendencia**, del tipo que exige precisamente una fuente institucional fechada, y
> **Francisco ha decidido retirarla, no reetiquetarla**. Las cuatro manifestaciones de doble uso
> enumeradas arriba se mantienen: son descriptivas y no dependían de esas citas. Las entradas `[37]`
> y `[45]` **no se borran ni se renumeran** en `Bibliografía.md`; quedan marcadas como sin uso en el
> texto. Para recuperar el cierre hace falta primero identificar el informe y su edición.
...
> - **La literatura de ejemplos adversariales** [46], [47], citada por el fenómeno general.
> - El **informe de agencia europea sobre IA y cibercrimen** [37], [45] **ya no figura aquí**: su edición nunca se pudo fijar y **las afirmaciones que sostenía se retiraron el 2026-08-18** (ver el callout de §2.3.2.3). No queda ningún uso de `[37]` ni de `[45]` en esta nota.
>
