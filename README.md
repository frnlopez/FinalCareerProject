...
| — | `agregar_semillas.py` | **Agregador de la dispersión (T4).** Convierte las tablas `metricas_*_semillas.csv` en `dispersion_semillas.csv` y `dispersion_semillas.md` (n, media, desviación típica muestral, mín y máx). En la misma pasada emite `comparaciones_pareadas.csv` (13 filas: los recuentos pareados por semilla de RandomForest vs HistGradientBoosting en `f1_macro` y de los dos ejes de balanceo, con fila por bloque y fila `__global__`); de esa tabla solo son citables las filas de firmas — las de balanceo son criterio de selección (4.3.4). Aborta si a alguna combinación le faltan semillas, y avisa —sin abortar— si una celda mezcla commits o si el balanceo ganador cambia entre semillas. Además compara el titular de la semilla 42 contra cada banda y emite él mismo el recuento «N de M celdas fuera». Para eso **abre las nueve tablas publicadas en LECTURA** (`pd.read_csv`, desde el 2026-08-13, solo para recuperar el valor de la 42): lo que sigue garantizado —y es lo que importa— es que **no escribe en ninguna**, comprobado por nombre **y por ruta** antes de empezar |
