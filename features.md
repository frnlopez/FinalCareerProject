...
|---|---|---|---|
| 2026-08-28 | Informe | **TERCERA PASADA DEL CRITERIO DE ACRÓNIMOS — CAMBIO DE CRITERIO. Continuación de las dos fichas inferiores (`2288227` y `27b60d9`).** Carril Intervención, **sin ficha previa en `## Abiertas`**. **(1) LO IMPORTANTE, CAMBIO DE CRITERIO a raíz de una objeción de Francisco** («¿debe ponerse el acrónimo en un título o en la leyenda de una tabla?»): **NO. Los encabezados de sección y los pies de figura y tabla son ELEMENTOS AUTÓNOMOS** —se leen fuera del hilo del texto, en el índice general, en el índice de tablas y al hojear— y la convención editorial los quiere **desarrollados**: un TOC que dijera «2.1 Introducción al ML» es peor documento. **Quedan FUERA del criterio de reincidencia, igual que ya estaban las celdas de tabla.** **COROLARIO SIMÉTRICO, y es lo que mueve los recuentos: si un encabezado o una leyenda no cuentan como REINCIDENCIA, tampoco cuentan como PRESENTACIÓN** — «presentación contigua» pasa a apoyarse **solo en la frase adyacente del cuerpo**. **(2) DECISIONES DE FRANCISCO APLICADAS: retiradas `IEEE`, `NSL-KDD`, `ADFA` y `SP`; el universo baja de 68 a 64 siglas.** Con ello quedan **resueltas todas las decisiones que le estaban pendientes** sobre altas y bajas, y la duda de los **paréntesis de emisor de `ADFA` y `SP` queda SIN OBJETO**. **(3) RESULTADO** sobre `Resultados/docx/REPASO_20260828-0650/` (`.docx` mtime **2026-08-28 06:46:51**): **16 cumplen / 48 fallan**, frente al 18/46 anterior. Reparto de fallos: **(a) nunca se presenta con su desarrollo = 33 · (b) se presenta y luego el documento vuelve al desarrollo = 12 · (d) el desarrollo aparece antes que la sigla = 3.** **(4) MOVIMIENTOS ENTRE COLUMNAS:** pasa a cumplir **`PR`** (su presentación está en un ítem de lista del cuerpo, no en un encabezado); dejan de cumplir **`LOF` → (a)**, que es el **caso puro del corolario** —su única presentación era el encabezado «Local Outlier Factor»—, y **`H-NIDS` → (b)** y **`KS` → (b)**, estas dos **no por la regla nueva sino por reincidencias reales en prosa**. **`RF`, `KNN` y `DL` cambian de motivo sin cambiar de columna, de (d) a (a)**, porque lo que sostenía el (d) era la Lista de Acrónimos o un pie —zonas ahora excluidas—: **el fallo es más grave de lo que parecía, no es orden sino AUSENCIA.** **(5) HALLAZGO QUE VALE COMO REGLA, confirmado ya dos veces: unificar un desarrollo a la sigla puede BORRAR la propia presentación y empeorar el fallo de (b) a (a).** Le pasó a **`CV`**, cuya unificación de «validación cruzada» fue tan completa que su primera aparición en prosa es hoy «se compara por CV», sin desarrollo. Por eso el entregable marca las sedes **NO TOCAR**: las de `ML`, `IA`, `FP`, `FN`, `FPR`, `VN`, `KS`, `MSE`, `MLP`, `DoS`, y **muy en particular la frase `Diseñar, implementar y evaluar un Sistema de Detección de Intrusiones Híbrido en Red (H-NIDS)`, que ES la presentación de H-NIDS** aunque parezca una reincidencia de IDS. **(6) DOS FRENTES POR SIGLA**, observación de Francisco incorporada al método: hay que barrer la forma en **ESPAÑOL** y la forma en **INGLÉS** del desarrollo — en `ML` él había unificado «aprendizaje automático» (0 ocurrencias vivas) pero **no «machine learning»** (1 viva en prosa). **(7) SIGLAS FANTASMA: `OCSVM` y `H-IDS` ya no aparecen en ninguna zona del documento**; se pueden retirar de la Lista de Acrónimos sin dejar apariciones huérfanas. **(8) PROBLEMA APARTE, de forma canónica y no de acrónimos:** conviven **`k-NN`** (3 en cuerpo, dos en zonas excluidas), **`K-NN`** (24 en cuerpo, 10 en tablas, 1 en pie) y **`KNN`** (0 en el texto; es la del código, `firmas.py`). **Recomendado unificar a `K-NN`** en prosa. **ENTREGABLE: el repaso completo se entregó EN EL CHAT, a petición expresa de Francisco** («deja los resultados en el chat, y yo mismo los revisaré en el informe para limpiarlo»), con sedes literales para Ctrl+F, agrupadas por motivo y separando reemplazo mecánico de revisión manual. **NO se editó el `.docx`.** **QUEDA ABIERTO:** aplicar las correcciones al `.docx` (lo hace Francisco a mano); decidir si las siglas de **nombre propio, organismo y protocolo** (`CIC-IDS2017`, `UNSW-NB15`, `KDD`, `MIT`, `NIST`, `DARPA`, `IP`, `TCP`, `ICMP`, `FTP`, `SNMP`, `CPU`, `GPU`, `CSV`, `AUT`, `RIPPER`) se presentan o **se retiran de la lista —recomendado retirarlas—**; y unificar la forma canónica de `K-NN`. **INCIDENCIA OPERATIVA: dos agentes murieron por límite de sesión antes de entregar este mismo encargo, y hubo que rehacerlo entero desde cero.** Ficheros tocados: `acronimos-para-word.txt` (modificado) y `Resultados/docx/REPASO_20260828-0650/` (**nuevo**, 8 ficheros) | `1aeeaef` |
| 2026-08-28 | Informe | **CONTINUACIÓN DEL BARRIDO DE ACRÓNIMOS — VERIFICACIÓN CONTRA EL `.docx` REAL. RESUELVE EL AVISO VIVO de la ficha inmediatamente inferior (`2288227`).** Carril Intervención, **sin ficha previa en `## Abiertas`**. **(0)** Word cerrado y **sin lock `~$`**: re-extracción válida a `Resultados/docx/REPASO_20260828-0555/` (**8 ficheros**), desde el `.docx` de **4.050.371 bytes**, mtime **2026-08-28 05:21:29**. Los recuentos **ya no están desfasados**. **(1) Resultado: de las 68 siglas, 67 aparecen EN PROSA, 1 solo fuera de prosa y 0 ausentes.** La única fuera de prosa es **`IEEE`** (12 ocurrencias, **todas en la bibliografía** como editorial o nombre de revista). Las **17 de una sola ocurrencia** (`SP`, `TPR`, `IPS`, `AUT`, `UEBA`, `CPU`, `GPU`, `ICMP`, `CSV`, `SRI`, `SIEM`, `RIPPER`, `ADFA`, `INCIBE`, `OCSVM`, `HIDS`, `H-IDS`) se verificaron **una a una a mano**: todas con sede real en el cuerpo. **Ninguna sigla nueva que dar de alta.** **(2) PREMISA CORREGIDA: es FALSO que el `.docx` creciera +21.110 bytes por texto añadido.** El cuerpo **ENCOGIÓ**: `documento.txt` pasó de **302.321 a 299.822 bytes**. Hubo **borrados reales** —los Agradecimientos de plantilla y **14 marcadores de cita, de 109 a 95**— más repaginado: **el fichero creció por formato, no por contenido**. **(3) HALLAZGO DE MÉTODO, vale como regla: los recuentos anteriores estaban INFLADOS** porque sumaban `tablas.txt` a `documento.txt`, y **las celdas aparecen en AMBOS ficheros** — `f1_macro` figuraba con **66** ocurrencias cuando el documento tiene **44**. **Todo recuento previo hecho así es alto.** **(4) Los 9 campos de forma, recontados sobre el documento real:** AUC-ROC 15 vs ROC-AUC 1 · AUC-PR 4 vs PR-AUC 1 · KNN 24 vs k-NN 2 · OneClassSVM 21 / One-Class SVM 6 / OCSVM 1 · DoS 7 vs DOS 2 · HIDS 1 vs H-IDS 1 · f1_macro 44 / F1_macro 3 / «F1 macro» 6 · AUC a secas 4 · Probe 5 vs PROBE 2. **Detectada una TERCERA forma no contemplada de AUC-ROC: `roc_auc` en monoespaciado** (el scorer de scikit-learn), presente en el cuerpo. **(5) Retirados los paréntesis de traducción/glosa: 46 de las 68 entradas modificadas.** Quedan **DOS con paréntesis a propósito, porque identifican al emisor y no traducen**: `ADFA` (Australian Defence Force Academy, organismo de origen del dataset) y `SP` (Special Publication, serie del NIST). **QUEDA ABIERTO, todo pendiente de Francisco:** (a) decidir si se retira `IEEE` de la lista; (b) decidir las **formas canónicas de los 9 campos**; (c) decidir sobre los **dos paréntesis de emisor**; (d) **aplicar la lista al `.docx`**. Ficheros tocados: `acronimos-para-word.txt` (modificado) y `Resultados/docx/REPASO_20260828-0555/` (**nuevo**, 8 ficheros) |`27b60d9` |
| 2026-08-28 | Informe | **BARRIDO COMPLETO DE ACRÓNIMOS DE LA MEMORIA — carril Intervención, SIN FICHA PREVIA en `## Abiertas`.** Barrido sobre la extracción `Resultados/docx/REPASO_20260827-1854/` (`documento.txt` y `tablas.txt`). **Entregable: `acronimos-para-word.txt`, fichero NUEVO en la raíz del repo**, en texto plano `SIGLA<tab>desarrollo`, **68 acrónimos** en orden alfabético y sin duplicados, **incluidos los 11 que ya estaban en la Lista de Acrónimos del `.docx`**: así el pegado **reemplaza el índice entero de una vez** en lugar de fusionarse a mano. Debajo de la lista, **9 campos a corregir**, que son **incoherencias de forma, no altas**: `AUC-ROC` vs `ROC-AUC`, `AUC-PR` vs `PR-AUC`, `KNN` vs `k-NN`, `OneClassSVM` vs `OCSVM`, `DoS` vs `DOS`, más **4 halladas en el propio barrido**; cada una con **recuento de ocurrencias y variante recomendada**. **La elección de la forma canónica queda PENDIENTE de Francisco: no se decidió aquí.** **`NSL-KDD` va marcado `[POR CONFIRMAR]`** porque sus autores no desarrollan la sigla. ~~**AVISO VIVO: los recuentos de ocurrencias salen de la extracción del 2026-08-27 18:54 y YA ESTÁN DESFASADOS** — el `.docx` tiene mtime **2026-08-28 05:21**; quien vaya a aplicar la lista debe **re-extraer y recontar antes**.~~ **AVISO RESUELTO el 2026-08-28 por la ficha de continuación inmediatamente superior: se re-extrajo y se recontó.** **Queda abierto:** aplicar la lista al `.docx` y decidir las formas canónicas de los 9 campos. Fichero tocado: `acronimos-para-word.txt` (nuevo). Ningún otro || `2288227` |
| 2026-08-27 | Informe | **`Las tareas de Kiko.md` PODADO tras el repaso contra la extracción NUEVA del `.docx`** (`Resultados/docx/REPASO_20260827-1812/`, mtime de fuente **2026-08-27T17:28:29**, al día y **sin fichero de bloqueo de Word**). Fichero único tocado (raíz del repo); la carpeta de extracción es artefacto nuevo. **(1) Borradas las 4 tareas ya resueltas: `6.12`** (la columna P1–P10 sin rótulo era **deliberada**), **`4.15`, `5.28` y `A13`** (las tres comprobadas **ya arregladas en Word** por Francisco). **(2) Recuentos corregidos: `T4`** de «~18 filas» a **5**; **cabecera del Anexo C** de «8 filas» a **5 filas / 8 marcadores**. **(3) Cinco tareas se MANTIENEN abiertas por verificación NEGATIVA en la extracción, con su prueba: `X31`** (`p2305` sigue con `[74]`), **`4.16`** (`p1073` sigue con comillas rectas), **`2.25`** (`p0659`/`p0660` vacíos existen), **`3.10`** (`p0741`–`p0744` existen) y las **5 filas del Anexo C** (marcadores de librerías ausentes, `p1867` solo lleva `[69]`, `p0643` y `p0877` sin marcador). Carril Intervención, **sin ficha previa en `## Abiertas`**: se registra aquí para que no desaparezca sin rastro | `—` |
| 2026-08-27 | Informe | **TAREA `B2` CORREGIDA en `Las tareas de Kiko.md` (raíz del repo): el protocolo de números QUEMADOS NO se aplica al `.docx`.** La afirmación «se borra `[9]` y NO se renumera nada» era **falsa** llevada al Word: los números quemados sin renumerar son la convención de `Bibliografía.md` del vault, que es **tabla manual**; la lista de referencias de Word **renumera sola** al borrar una entrada intermedia. Lo señaló Francisco. **Verificado contra la extracción `REPASO_20260826-2123`:** `[75]` = Moustafa y Slay (UNSW-NB15) es la **última fila** de la lista y tiene **una única cita en el cuerpo**, así que moverla al hueco del `[9]` cuesta **una sola sustitución de marcador**. En `B2` se escribió el **procedimiento paso a paso**, la **contradicción con `B1`** (que pide mantener la lista intacta) y la **alternativa de coste cero**: no borrar `[9]` y dejar el duplicado de Stallings. **La elección entre las dos opciones queda PENDIENTE de Francisco**; `B1` no se tocó. Carril Intervención, **sin ficha previa en `## Abiertas`**: se registra aquí para que no desaparezca sin rastro | `cf12ddb` |
| 2026-08-27 | Informe | **`Las tareas de Kiko.md` REESCRITO DE ARRIBA ABAJO: 667 → 236 líneas.** Fichero único tocado (raíz del repo). **(1) Retirados TODOS los callouts de warning.** **(2) Cerradas y borradas por estar verificadas en disco: `A5`, `B5`, `5.3`, `B3`, `5.24` y el recorte de `6.11`.** **(3) Contestados dentro del propio fichero los comentarios que Francisco había dejado bajo las tareas abiertas:** texto listo para pegar en `D1`/`5.2`, entrada IEEE de scikit-learn en `T3`, diccionario `Q1`/`C`, `H-2`, `H-3` y `H-6` en `5.4`, las cuatro entradas IEEE de `B2`, y las sedes de `2.11`, `2.12`, `6.9` y `X31`. **(4) Los antiguos Anexos C y D se FUNDEN en uno solo y se podan de 74 a 18 filas:** se separaron las citas de obras **aún no presentes en la prosa** (18 filas, que corresponden a las 19 entradas huérfanas de `Bibliografía.md`) de las de obras **ya citadas al menos una vez** (~56 filas), **estas últimas borradas por decisión del usuario**. **Nunca tuvo ficha en `## Abiertas`**; se registra aquí para que no desaparezca sin rastro | `0d5a76c` |
| 2026-08-26 | Informe | **BARRIDO `.docx` ↔ vault — COMPLETO, LOS SEIS CAPÍTULOS. FICHA CERRADA** (venía abierta desde el 2026-08-26 como «CONTINUACIÓN: capítulo 6 y el resto»). Resultado final en `Lo que quedo fuera.md` (raíz): **39 hallazgos** — cap. 5 → 8 · cap. 6 → 3 · cap. 4 → 8 · cap. 3 → 7 · cap. 2 → 8 (tope) · cap. 1 → 5. **Último ciclo (caps. 2 y 1):** **capítulo 2 EN EL TOPE, es el capítulo del que más masa de texto quedó atrás** —el `.docx` conserva la exposición pero suelta casi todas las tablas comparativas y los cierres de sección—, con **dos ausencias de SECCIÓN ENTERA visibles en el propio índice del `.docx`: `2.1.6 Metodologías y buenas prácticas` NO EXISTE** (salta de `2.1.5` a `2.2`) y **`2.3.3` se corta tras los tres pasos de P10, dejando fuera SEIS de sus ocho apartados**; excluido el material de los Anexos C.4–C.6 y D de `Las tareas de Kiko.md`. **Su auditoría salió `NO APTO` y no fue trámite:** el hallazgo 8 atribuía al `.docx` una frase con **cero ocurrencias** que además **contradecía frontalmente al hallazgo 1 del capítulo 6 ya escrito**; se corrigieron también una prueba falsa («jerarquía» no tenía cero ocurrencias sino dos, ajenas), dos sobre-alcances, un ancla de inserción inexistente y el recuento cinco→seis de `2.3.3`. **Capítulo 1 con 5 hallazgos, por debajo del tope y justificado:** el patrón es otro —**el `.docx` conserva una versión anterior y más pobre de tres de las cinco notas**, concentrada en `1.3 Objetivo`—; auditoría `APTO CON CAMBIOS`, las cinco ausencias confirmadas y ninguna retirada. Evidencia principal: **el capítulo 6 numera nueve objetivos y promete correspondencia «verificable uno a uno» con un `1.3` que solo publica siete, sin numerar**. **Decisión de Francisco vigente en todo el barrido: el vault NO se arregla** — los defectos se anotan en `Lo que quedo fuera.md` y ahí se quedan; ningún agente editó notas del vault, `resumen-de-decisiones.md`, `Las tareas de Kiko.md` ni el `.docx`. Ciclos previos: `9bf3c9a` (cap. 5), `3259c33` (cap. 6 + re-verificación del 5 + renombrado del `.docx`), `37445dd` (caps. 4 y 3) | `4589464` |
| 2026-08-26 | Informe | **EFECTO COLATERAL DEL ÚLTIMO CICLO SOBRE EL CAPÍTULO 5, YA CERRADO: el hallazgo 4 del cap. 5 partía de una PREMISA FALSA.** La auditoría del capítulo 2 confirmó que **el `.docx` no plantea en ningún sitio la dicotomía «reproducir la jerarquía confirma / contradecirla obliga a sospechar de fuga o artefacto»**. Se reescribió la premisa; **el resto del hallazgo sigue en pie**. Es la **única** edición hecha fuera de los capítulos 2 y 1 | `4589464` |
| 2026-08-26 | Informe | **AVISO REGISTRADO — TRAMPA DE NUMERACIÓN (hallazgo 5 del cap. 1):** Snort es **`[26]` en el vault** y **`[11]` en el `.docx`**; pero en la numeración del `.docx` **`[26]` es LightGBM (Ke et al.)**, así que trasladar el `[26]` tal cual **no dejaría una cita huérfana sino una cita FALSA**. Se registra como aviso, **no se resuelve** | `4589464` |
| 2026-08-26 | Informe | **AVISO REGISTRADO — CANDIDATO QUE NO CUPO EN EL TOPE DEL CAP. 2:** el **bloque ético completo de `2.3.3`** (explicabilidad, privacidad, RGPD/AI Act/NIS2). Es **ausencia real** y queda como **primer candidato** si Francisco amplía el tope de 8 | `4589464` |
| 2026-08-26 | Informe | **BARRIDO `.docx` ↔ vault · CAPÍTULO 6 — CERRADO, y CAPÍTULO 5 RE-VERIFICADO (segundo ciclo del barrido por capítulos).** Trabajo sobre la ficha de continuación abierta ese mismo día, que **sigue abierta**: quedan los capítulos 1, 2, 3 y 4 (y Apéndices si procede). **(0) Re-extracción del `.docx` con Word cerrado** → `Resultados/docx/REPASO_20260826-1908/` (**8 artefactos**; `.docx` de mtime **19:06**, **4.038.568 bytes**, **3.035 párrafos**, **48 tablas**, **23 figuras**, **93 marcadores**, **109 párrafos de TOC**). Era obligatoria: la extracción previa (`REPASO_20260826-0643`) iba **12 horas desfasada**. **Diferencia real entre ambas: +2 párrafos y nada de tablas, figuras ni citas**; el `Imágenes VML: 1 → 0` es un **criterio de conteo distinto del extractor, no contenido perdido** — que nadie lo fiche como pérdida. **(1) PASE A · capítulo 5 re-verificado contra la extracción fresca: sigue en 8 hallazgos, ninguno se retira**, **cuatro se ACOTAN** recortando la parte que la extracción refuta, y **uno que se había dado por caído se REPONE acotado** (el callout «Qué NO explica este 13,4 %», reducido a la mitad del punto de operación). **Con esto queda SUBSANADO el límite conocido del ciclo anterior:** la ausencia en el `.docx` ya está **verificada**, cosa que el `auditor-ml` no pudo hacer entonces con el documento abierto en Word. **(2) PASE B · capítulo 6 cerrado con 3 hallazgos, muy por debajo del tope de 8. NO es omisión:** se contrastó expresamente y **`6.1.1`–`6.1.3` y `6.2.1`–`6.2.6` están volcadas casi frase a frase**; excluido por tema lo ya fichado en `Las tareas de Kiko.md` (`6.4`, `6.5`, `6.6`, `6.7`, `6.9`, `6.10`, `6.11`, `6.12`, `B1` y `A14`). **(3) `auditor-ml`: `APTO CON CAMBIOS`, 6 defectos, CUATRO de ellos ausencias que la extracción refutaba** — el FPR de diseño y su causa **SÍ** estaban publicados; el argumento de la firma de la función **SÍ** está en el §3; las celdas de la variante 54 **SÍ** están en la Tabla 15; y el CAE estaba mal declarado. **Los 6 corregidos antes de cerrar.** Ficheros tocados: `Lo que quedo fuera.md`, `CLAUDE.md` y `Resultados/docx/REPASO_20260826-1908/` (**nuevo**) | `3259c33` |
| 2026-08-26 | Harness | **EL `.docx` DE LA MEMORIA SE RENOMBRA EN `CLAUDE.md`: `TFG - Fran.docx` ES NOMBRE MUERTO.** La memoria oficial es **`Proyecto_Fin_de_Grado-FJLM-2026.docx`**. Decisión de Francisco del 2026-08-26. Corregidas **las dos sedes** del fichero: el árbol de estructura del repositorio y la sección propia. **De paso queda documentada ahí la regla de extracción:** antes de comparar, **verificar el `mtime` del `.docx` contra el `_PROCEDENCIA.txt` de la última extracción**, y un fichero **`~$…` significa Word abierto y re-extracción INVÁLIDA**. Fichero tocado: `CLAUDE.md` | `3259c33` |
| 2026-08-26 | Ninguno | **INCIDENCIA DEL ANDAMIAJE (se registra, no se resuelve): `SendMessage` estaba DESHABILITADO en la sesión del segundo ciclo del barrido.** No se pudo **reanudar al `redactor-tfg` original con su contexto** para aplicar las correcciones del `auditor-ml`, así que hubo que **despachar uno nuevo con encargo autocontenido**. Funcionó, pero **encarece cada ciclo de corrección** y choca con la nota de memoria «Corte de sesión: reanudar, no relanzar», que presupone `SendMessage` disponible | `3259c33` |
| 2026-08-26 | Informe | **BARRIDO `.docx` ↔ vault · CAPÍTULO 5 — CERRADO (primer ciclo de un barrido por capítulos).** Comparación de `Proyecto_Fin_de_Grado-FJLM-2026.docx` contra el informe del vault (00–06 + Apéndices) para rescatar contenido de alto valor perdido en el recorte del traspaso. Produce `Lo que quedo fuera.md` (raíz del repo) con **8 hallazgos del capítulo 5** (tope acordado: 8 por capítulo). Criterio de admisión: entra un hallazgo solo si su ausencia (a) rompe una afirmación que el `.docx` sí hace, (b) elimina una contribución propia del trabajo, o (c) deja sin respaldo una conclusión del cap. 6; se excluye por tema lo ya recogido en `Las tareas de Kiko.md`. **Solo lectura** sobre el vault y el `.docx`: no se tocó código ni notas de la memoria. **Nunca tuvo ficha en `## Abiertas`** (carril Intervención), y se registra aquí para que no desaparezca sin rastro. **Auditado por `auditor-ml`: cifras y sedes verificadas, pero NO pudo verificar de forma independiente la ausencia en el `.docx`** — el documento estaba abierto en Word (`~$` presente). **Quedan el cap. 6 y el resto: ficha abierta para la continuación** | `9bf3c9a` |
| 2026-08-26 | Informe | **DESVÍO ANOTADO AL MARGEN del barrido del cap. 5: la Tabla 5.10 del `.docx` NO cuadra con el disco.** Celda `apache2` × IsolationForest dice **`0.955`**; el vault y `metricas_hibrido_0day.csv` dan **`0.995`**. Se registra como incoherencia detectada, **no resuelta**: la corrección en el `.docx` es de Francisco | `9bf3c9a` |
| 2026-08-26 | Informe | **🟠 `Las tareas de Kiko.md` REESCRITO DE ARRIBA ABAJO: DE LEDGER NARRATIVO A LISTA DE TAREAS EJECUTABLE — carril Tarea, track Informe, SIN FICHA PREVIA en `## Abiertas`.** Lo pidió Francisco: «saca lo que realmente está roto o mal … deja de adornar tanto cada tarea por hacer, sé conciso, sé directo y sé claro. Que es lo que hace falta hacer HOY. Qué es parte del recorte. Qué es parte de mejorar el informe … quita de las tareas de kiko todas las cosas que no sean tareas, con eso me refiero a TODAS las notas, los info y demás», y añadió que era **la última vez que lo pedía**. **Re-extracción OBLIGATORIA: el `.docx` había cambiado otra vez** (mtime **2026-08-26 06:43:23**, 4.038.690 bytes, frente a los 4.025.496 del volcado de las 06:04), así que **toda la verificación sale de `Resultados/docx/REPASO_20260826-0643/` (8 ficheros)**. **Resultado: 754 → 314 líneas, 89 → 77 casillas, 14 → 0 callouts.** Estructura nueva en **tres bloques** que sustituye a la organización por capítulos: `## HOY · Roto o mal` (lo que un tribunal vería como fallo), `## Recorte` (redundancia) y `## Mejora` (nada urgente). **Formato de tarea reducido a UNA línea:** fragmento literal de Ctrl+F, acción en imperativo y etiqueta `Word`/`vault`, con el texto de antes y de después cuando es un reemplazo. Fuera toda la prosa explicativa, las justificaciones, los censos y las declaraciones de frontera. Los **Anexos C (26 filas) y D se conservan** porque cada fila es una cita por pegar, pero sin su prosa. **15 tareas retiradas, todas justificadas contra el volcado del 06:43:** `T5` (wikilinks) tiene **CERO ocurrencias de `[[` en el `.docx`** —Francisco los quitó todos— y `T9` **CERO ocurrencias del placeholder «EDAKJDFSLDLKJ»**; **once se CONSOLIDARON dentro de `B1`, no se perdieron** (`2.19`-`2.23`, `3.7`, `4.14`, `5.25` y `5.26` eran todas la misma decisión —qué hacer con las obras que están en la lista bibliográfica sin cita en el cuerpo— y hoy son sub-viñetas: Bergstra y Bengio, TabArena/Micci-Barreca/Pargent, Hwang, Pendlebury, Behrouz, Barbará/Grinsztajn/McElfresh/Shwartz-Ziv/Hollmann, CRISP-DM, Storkey y Mitchell); `2.24`, `3.1` y `4.13` se **reubicaron** y siguen fichadas. **`3.9` es la ÚNICA baja deliberada**: «la Figura 3.1 está en el `.docx` pero fuera del censo de `assets/` del vault» no encajaba en ninguno de los tres bloques. **Queda PENDIENTE de que Francisco decida si se restituye** — es una incoherencia real entre el `.docx` y el censo del vault, no un defecto del documento. **Tres tareas nuevas** en el bloque de recorte: `R7`, `R8`, `R10`. **Verificación de calidad hecha en el hilo principal, NO delegada** (el `auditor-ml` murió dos veces por límite de cuota y su pase quedó sin veredicto): se comprobaron **221 fragmentos de Ctrl+F** contra `documento.txt` e `indice_toc.txt`, y **tres eran inservibles y se corrigieron** — **`X31`**, cuya sede ya no existía porque Francisco reescribió la frase (era «la magnitud del problema sin medirla. El dato citable es el de [74]» y hoy es «ilustra la magnitud del problema sin medirla. [74]»; la decisión `[74]` vs `[72]` **sigue abierta**, pero el fragmento no encontraba nada); **`T8`**, cuya entrada de índice es «A.1. Columnas del dataset NSL-KDD**167**», no «172»; y la sub-viñeta de **`5.5`**, cuyo fragmento «la evidencia de Resultados/metricas_hibrido_0day.csv» no coincidía con el texto real, «del autoencoder (Resultados/metricas_hibrido_0day.csv». **Estado final verificado: 77 casillas, 0 callouts, 3 bloques, 314 líneas y CERO sedes que no existan en el cuerpo o en el índice.** Ficheros tocados: `Las tareas de Kiko.md` y `Resultados/docx/REPASO_20260826-0643/` (**nuevo**, 8 ficheros) |`66692a3` + `05a7207` |
