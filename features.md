...
### 🔎 BARRIDO `.docx` ↔ vault — CONTINUACIÓN: capítulo 6 y el resto (abierta el 2026-08-26)

- [~] **Seguir el barrido de comparación entre `Proyecto_Fin_de_Grado-FJLM-2026.docx` y el informe
  completo del vault (00–06 + Apéndices)**, para detectar contenido de alto valor que se perdió en
  el recorte del traspaso. **El capítulo 5 quedó CERRADO el 2026-08-26** (8 hallazgos en
  `Lo que quedo fuera.md`, raíz del repo). **El capítulo 6 quedó CERRADO el 2026-08-26 en el segundo
  ciclo** (3 hallazgos, muy por debajo del tope de 8, y no por omisión: `6.1.1`–`6.1.3` y
  `6.2.1`–`6.2.6` están volcadas casi frase a frase). En ese mismo ciclo **el capítulo 5 se
  RE-VERIFICÓ** contra una extracción fresca con Word cerrado (`Resultados/docx/REPASO_20260826-1908/`):
  sigue en 8 hallazgos, cuatro acotados y uno repuesto. **Quedan los capítulos 1, 2, 3 y 4** (y
  Apéndices si procede).
  - **Es un barrido POR CAPÍTULOS, uno por ciclo, con aprobación de Francisco entre medias.**
    Orden acordado: **cap. 5 → cap. 6 → el resto**. No se encadenan dos capítulos sin su visto bueno.
  - **Criterio de admisión** (el mismo del cap. 5): entra un hallazgo solo si su ausencia (a) rompe
    una afirmación que el `.docx` sí hace, (b) elimina una contribución propia del trabajo, o
    (c) deja sin respaldo una conclusión del cap. 6. **Tope de 8 hallazgos por capítulo.**
    **Se excluye por tema lo ya recogido en `Las tareas de Kiko.md`.**
  - **Solo lectura** sobre el vault y el `.docx`. El barrido no toca código ni notas de la memoria:
    lo único que escribe es `Lo que quedo fuera.md`.
  - ~~**Límite conocido del método, heredado del cap. 5:** el `auditor-ml` **no puede verificar de
    forma independiente la AUSENCIA de un contenido en el `.docx`** mientras el documento esté
    abierto en Word (`~$` presente). Verifica cifras y sedes; la ausencia queda declarada, no
    probada.~~ **— SUBSANADO el 2026-08-26 en el segundo ciclo:** con Word cerrado la extracción es
    válida y la ausencia **sí** se verifica. Que sirva de método para los capítulos que quedan:
    **antes de comparar, cerrar Word, re-extraer y cotejar el `mtime` contra el `_PROCEDENCIA.txt`
    de la última extracción**; un `~$…` en la carpeta significa re-extracción inválida. En ese ciclo
    **cuatro de los seis defectos del `auditor-ml` fueron ausencias que la extracción refutaba**, así
    que el riesgo no es teórico.
