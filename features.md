...
| 2026-08-26 | Harness | **EL `.docx` DE LA MEMORIA SE RENOMBRA EN `CLAUDE.md`: `TFG - Fran.docx` ES NOMBRE MUERTO.** La memoria oficial es **`Proyecto_Fin_de_Grado-FJLM-2026.docx`**. Decisión de Francisco del 2026-08-26. Corregidas **las dos sedes** del fichero: el árbol de estructura del repositorio y la sección propia. **De paso queda documentada ahí la regla de extracción:** antes de comparar, **verificar el `mtime` del `.docx` contra el `_PROCEDENCIA.txt` de la última extracción**, y un fichero **`~$…` significa Word abierto y re-extracción INVÁLIDA**. Fichero tocado: `CLAUDE.md` | — |
| 2026-08-26 | Ninguno | **INCIDENCIA DEL ANDAMIAJE (se registra, no se resuelve): `SendMessage` estaba DESHABILITADO en la sesión del segundo ciclo del barrido.** No se pudo **reanudar al `redactor-tfg` original con su contexto** para aplicar las correcciones del `auditor-ml`, así que hubo que **despachar uno nuevo con encargo autocontenido**. Funcionó, pero **encarece cada ciclo de corrección** y choca con la nota de memoria «Corte de sesión: reanudar, no relanzar», que presupone `SendMessage` disponible | — |
