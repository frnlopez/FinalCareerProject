...
- [ ] 🟠 **La frase de `4.5` sobre KNN («difieren marginalmente») es FALSA — y lo es en LAS DOS variantes** · Informe · `redactor-tfg`
  Alta el **2026-08-18**, hallazgo de la TANDA B del Grupo IV, **verificado contra `Resultados/` dos
  veces**: por el agente y de forma independiente en hilo principal leyendo los CSV.
  **No difieren: son el mismo número.** `metricas_balanceo.csv` (fila `KNN`/`SMOTE`) y
  `metricas_firmas.csv` (fila `KNN`) dan **`f1_macro_cv = 0,945349` en la variante de 54** y
  **`0,952282` en la de 122**, idénticos hasta el último dígito en ambas. **El informe de la TANDA B
  solo señalaba la de 54: el solape afecta a LAS DOS.** Causa: ambas filas salen del **mismo commit
  `1163c90`** y la `config_ganadora` es `{'clf__n_neighbors': 5, 'clf__weights': 'uniform'}` —la
  rejilla eligió **el defecto**, que es la configuración con la que ya había corrido el experimento
  de balanceo—. **HistGradientBoosting sí difiere, pero por REDONDEO** (`0,969377` vs `0,969391`,
  `1,4e-5`). Es **una cifra falsa publicada en la memoria**. Ya tuvo ficha —cerrada el 2026-08-16 en
  la TANDA 14 con estas mismas cifras exactas—, pero **la corrección nunca bajó a la prosa de `4.5`**:
  se cerró el diagnóstico, no la redacción. **No se toca ninguna cifra de los CSV: se reescribe la
  frase.**
